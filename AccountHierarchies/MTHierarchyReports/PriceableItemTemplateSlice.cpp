...
STDMETHODIMP CPriceableItemTemplateSlice::InterfaceSupportsErrorInfo(REFIID riid)
{
	static const IID* arr[] = 
	{
    &IID_IPriceableItemTemplateSlice,
...
STDMETHODIMP CPriceableItemTemplateSlice::ToString(BSTR *pVal)
{
	if (!pVal)
		return E_POINTER;
	else
...
STDMETHODIMP CPriceableItemTemplateSlice::ToStringUnencrypted(BSTR *pVal)
{
	if (!pVal)
		return E_POINTER;
	else
...
STDMETHODIMP CPriceableItemTemplateSlice::FromString(ISliceLexer* apLexer)
{
	if (!apLexer)
		return E_POINTER;
	try
...
STDMETHODIMP CPriceableItemTemplateSlice::Equals(IViewSlice* apSlice, VARIANT_BOOL *pVal)
{
	if(!pVal)
		return E_POINTER;
	else
	...
	try
	{
		...
		if(pSlice)
		{
			MTHIERARCHYREPORTSLib::IPriceableItemTemplateSlicePtr pCast(pSlice);
			if(NULL != pCast &&
...
STDMETHODIMP CPriceableItemTemplateSlice::Clone(IViewSlice* *pVal)
{
	if(!pVal)
		return E_POINTER;
	else
	...
	try
	{
		MTHIERARCHYREPORTSLib::IPriceableItemTemplateSlicePtr This(this);
		MTHIERARCHYREPORTSLib::IPriceableItemTemplateSlicePtr clone(__uuidof(MTHIERARCHYREPORTSLib::PriceableItemTemplateSlice));
		clone->TemplateID = This->TemplateID;
		clone->ViewID = This->ViewID;
		*pVal = reinterpret_cast<IPriceableItemTemplateSlice *> (clone.Detach());
	}
	catch(_com_error & e)
	{
		return returnHierarchyReportError(e);
...
STDMETHODIMP CPriceableItemTemplateSlice::get_TemplateID(long *pVal)
{
	if(!pVal)
		return E_POINTER;
	else
...
STDMETHODIMP CPriceableItemTemplateSlice::put_TemplateID(long newVal)
{
	mTemplateID = newVal;
...
STDMETHODIMP CPriceableItemTemplateSlice::get_ViewID(long *pVal)
{
	if(!pVal)
		return E_POINTER;
	else
...
STDMETHODIMP CPriceableItemTemplateSlice::put_ViewID(long newVal)
{
	mViewID = newVal;
...
STDMETHODIMP CPriceableItemTemplateSlice::GenerateQueryPredicate(BSTR *pVal)
{
	if (!pVal)
		return E_POINTER;
	else
	...
	try
	{
		if (mViewID == -1)
		{
			return Error("PriceableItemTemplateSlice not initialized properly: ViewID must be set");
		}
		if (mTemplateID == -1)
		{
			return Error("PriceableItemTemplateSlice not initialized properly: TemplateID must be set");
		}
		QUERYADAPTERLib::IMTQueryAdapterPtr pQueryAdapter(MTPROGID_QUERYADAPTER) ;
		pQueryAdapter->Init(L"\\Queries\\PresServer");
		pQueryAdapter->SetQueryTag(UseDataMart() ? L"__PRICEABLE_ITEM_TEMPLATE_PREDICATE_DATAMART_PRESSERVER__" : L"__PRICEABLE_ITEM_TEMPLATE_PREDICATE_PRESSERVER__");
    ...
    out = pQueryAdapter->GetQuery() +  
      SingleProductViewSliceImpl<IPriceableItemTemplateSlice, 
      &IID_IPriceableItemTemplateSlice, 
...
STDMETHODIMP CPriceableItemTemplateSlice::get_ProductView(IProductView* *pVal)
{
	if (!pVal)
		return E_POINTER;
	else
...
STDMETHODIMP CPriceableItemTemplateSlice::get_DisplayName(ICOMLocaleTranslator *apLocale, BSTR *pVal)
{
	if(!pVal)
		return E_POINTER;
	else
