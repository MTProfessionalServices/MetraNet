---------------------------------------------------------------
-- Run this whole file to create the Account Hierarchy
-- stored procedures, tables, and seed the t_account_ancestor 
-- for creation of the MetraTech Hierarchy test accounts
---------------------------------------------------------------
...
--SET NOCOUNT ON
	...
	vt_start <= @dt_start AND @dt_start < vt_end AND tt_end = dbo.MTMaxDate()
...
declare @status as int
select * from 	t_account_ancestor where id_descendent = 0
...
-----------------------------------------------------------
-- remove existing entries in t_account_ancestor
-----------------------------------------------------------
truncate table t_account_ancestor
go
-----------------------------------------------------------
-- drop old procedure
-----------------------------------------------------------
drop proc AddAccToHierarchyByName
go
-----------------------------------------------------------

-----------------------------------------------------------
--- used to populate and seed the MetraTech hierarchy
-----------------------------------------------------------
create proc AddAccToHierarchyByName(@nm_ancestor as varchar(255),@nm_descendent as varchar(255), @namespace as varchar(255))
                   --@status int OUTPUT)
as
 SET NOCOUNT ON
declare @ctime as datetime
declare @dt_start as datetime
declare @dt_end as datetime
declare @id_ancestor as int
declare @id_descendent as int
 set @ctime = GetUTCDate()
set @dt_start = '1/1/1753'
set @dt_end = '1/1/9999'
-- set @status = 0
set @id_ancestor = 9999
set @id_descendent = 10000
set @id_ancestor = CASE WHEN (@nm_ancestor = '1') Then 1 ELSE (SELECT id_acc FROM t_account_mapper am WHERE am.nm_login = @nm_ancestor AND am.nm_space = @namespace) END
set @id_descendent = (SELECT id_acc FROM t_account_mapper am WHERE am.nm_login = @nm_descendent AND am.nm_space = @namespace)
...
id_ancestor,id_descendent,num_generations,vt_start,vt_end,tt_start,tt_end)
  -- create a record of all the parents above us in the hierarchy
  select id_ancestor,@id_descendent,num_generations + 1,
  vt_start,vt_end,@ctime,dbo.MTMaxDate() from
  t_account_ancestor where
