<%
  ...
  marrOptions = Array("By Charge", "By Account")
  marrValues = Array("BYCHARGE", "BYACCOUNT")
  ...
  Function RenderPage(strType)
    ...
    if UCase(strType) = "BYCHARGE" or len(strType) = 0 then
      Call objXSL.Load(server.MapPath("/prototypes/UI/MPS/ByCharge.xsl"))
      Call objXML.Load(server.MapPath("/prototypes/UI/MPS/Charges.xml"))
    elseif UCase(strType) = "BYACCOUNT" then
      Call objXSL.Load(server.MapPath("/prototypes/UI/MPS/ByCharge.xsl"))
      Call objXML.Load(server.MapPath("/prototypes/UI/MPS/Accounts.xml"))
    end if
...
<html>
  ...
  <body alink="darkgoldenrod">
    <form name="frmDisplay" action="bill.asp" method="POST">
