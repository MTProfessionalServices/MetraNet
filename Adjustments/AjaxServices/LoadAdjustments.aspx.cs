...
public partial class Adjustments_AjaxServices_LoadAdjustments : MTListServicePage
{
  private const int MAX_RECORDS_PER_BATCH = 50;

  protected bool ExtractDataInternal(AdjustmentsServiceClient client, ref MTList<AdjustedTransaction> items, int batchID, int limit)
  {
    try
    {
      items.Items.Clear();
      ...
      client.GetAdjustedTransactions(ref items);
    }
  ...
  protected bool ExtractData(AdjustmentsServiceClient client, ref MTList<AdjustedTransaction> items)
  {
  ...
  {
    using (new HighResolutionTimer("GetAdjustedTransactions", 5000))
    {
      ...
      try
      {
        ...
        var items = new MTList<AdjustedTransaction>();
