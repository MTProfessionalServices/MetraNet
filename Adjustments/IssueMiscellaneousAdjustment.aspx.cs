  ...
  private RCD.IMTRcd rcd = new RCD.MTRcd();
    ...
    private string GetMaxCapabilityAmount()
    {
        ...
        foreach (ApplyAdjustmentsCapability cap in capabilites)
        {
            ...
            amount = amount + String.Format(" {0} {1} {2} {3}", GetLocalResourceObject("TEXT_MAX_AUTHORIZED_AMOUNT"), ((InternalView)UI.Subscriber.SelectedAccount.GetInternalView()).Currency, cap.GetAtomicDecimalCapability().GetParameter().Test, display);
        }
