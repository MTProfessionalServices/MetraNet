...
{
  public partial class LoadAccountConfigSetList : MTListServicePage
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      using (new HighResolutionTimer("LoadAccountConfigSetList", 5000))
      {
        try
        {
          var items = new MTList<AccountConfigSet>();
    ...
    protected bool ExtractData(ref MTList<AccountConfigSet> items)
    {
      if (Page.Request["mode"] == "csv")
      {
    ...
    protected bool ExtractDataInternal(ref MTList<AccountConfigSet> items, int batchID, int limit)
    {
      items.Items.Clear();
      ...
      try
      {
        GetAccountConfigSets(ref items);
      }
    ...
    private void GetAccountConfigSets(ref MTList<AccountConfigSet> items)
    {
      var client = GetClientAccountConfigSet();
      client.InOut_acsList = items;
      client.Invoke();
      items = client.InOut_acsList;
    }

    private AccountConfigurationSetService_GetAccountConfigSetList_Client GetClientAccountConfigSet()
    {
      return new AccountConfigurationSetService_GetAccountConfigSetList_Client
        {
