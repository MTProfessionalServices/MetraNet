...
sub ProcessCommand
{
	...
	elsif ($command =~ /^(.*)(\((\d+)\))? : (warning|error|fatal error) ([^:]+): (.*)/)
	{
		...
		{
			print "IGNORE WARNING FOLLOWING!!!!!!!\n";
			print "WARNINGSource  $source\n";
			print "WARNINGCode    $code\n";
			print "WARNINGString  $string\n";
		}
	...
	{
		# Filter all other messages that are not checked for special processing
		my $ignore = 0;
		if ($command =~ /^cl .*[D9035](.*$)/)
		{
			#print "TRW Ignore D9035 oldsyntax!!!!!!! $command\n";
			$ignore = 1;
		}
		elsif ($command =~m/boost/ and $command =~ m/ warning: / )
		{
			#print "TRW Ignore BOOST Warning!!!!!! $command\n";
			$ignore = 1;
		}
		else
		{
