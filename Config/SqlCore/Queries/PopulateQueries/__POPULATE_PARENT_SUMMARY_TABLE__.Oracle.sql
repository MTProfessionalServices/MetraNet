	...
	begin
   if table_exists('%%NETMETER_DB_NAME%%.t_pv_audioconfcall') then
   EXECUTE IMMEDIATE ('
	insert into t_rpt_parent_summary
	/*  Conferences */
	select inv.id_invoice InvoiceID,
	au.id_payee PayeeIDAcc,
	nvl(PayeeContact.c_FirstName || '' '', '''') ||
	nvl(PayeeContact.c_MiddleInitial || '' '', '''') ||
	nvl(PayeeContact.c_LastName, '''') PayeeName,
	...
	au.amount + nvl(au.tax_federal, 0) + nvl(au.tax_state, 0) + nvl(au.tax_county, 0) + nvl(au.tax_local, 0) + nvl(au.tax_other, 0) ConfAmount,
	...
	from %%NETMETER_DB_NAME%%.t_invoice inv
		...
		inner join %%NETMETER_DB_NAME%%.t_enum_data edLanguage  /* Use the payers language for localized settings */
