...
where aa.id_descendent = %%accountId%%
  and TO_DATE(%%refDate%%,'YYYY-MM-DD HH24:MI:SS') between aa.vt_start and aa.vt_end
  and aa.num_generations = 
    (select max(num_generations)
    from t_account_ancestor 
    where id_descendent = %%accountId%%
      and TO_DATE(%%refDate%%,'YYYY-MM-DD HH24:MI:SS') between vt_start and vt_end
      and id_ancestor <> 1)