...
int CMTCryptoAPI::Initialize(MetraTech_Security_Crypto::CryptKeyClass keyClass, const std::string& szContainerName, bool bMachineKey, const std::string& szSource)
{
  ...
  cryptoManager = new MetraTech_Security_Crypto::ICryptoManagerPtr(__uuidof(MetraTech_Security_Crypto::CryptoManager));
...
int CMTCryptoAPI::Decrypt(std::string& ciphertext)
{
  ...
  ciphertext = cryptoManager->Decrypt(cryptKeyClass, _bstr_t(ciphertext.c_str()));
  return 0;
}
...
int CMTCryptoAPI::Decrypt(char * pData, long * pDataLen)
{
  ...
  string ciphertext(data, *pDataLen);
  ...
  {
    delete[] pData;
    pData = new char[ciphertext.size()+1];
  	// copy the string
	  std::strcpy(pData, ciphertext.c_str());
    *pDataLen = ciphertext.length();
  }
...
{
  plaintext = cryptoManager->Encrypt(cryptKeyClass, _bstr_t(plaintext.c_str()));
  return 0;
}
...
int CMTCryptoAPI::Encrypt(char * pData, long * pDataLen, long lBufLen)
{
  ...
  {
    delete[] pData;
    pData = new char[plaintext.size()+1];
  	// copy the string
	  std::strcpy(pData, plaintext.c_str());
    *pDataLen = plaintext.length();
  }
...
int CMTCryptoSessionKey::MapAlgorithm(const std::string& sEncryptionType, ALG_ID& aiAlgId)
{
  ...
  if (sEncryptionType == "AES")
  {
		aiAlgId = CALG_AES_256;
  }
	else if (sEncryptionType == "RC2")
	{
		aiAlgId = CALG_RC2;
	}
