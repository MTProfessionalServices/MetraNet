#ifndef INC_MTSQLOracleParser_hpp_
#define INC_MTSQLOracleParser_hpp_

#include <antlr/config.hpp>
/* $ANTLR 2.7.6 (2005-12-22): "expandedmtsql_parser_oracle.g" -> "MTSQLOracleParser.hpp"$ */
#include <antlr/TokenStream.hpp>
#include <antlr/TokenBuffer.hpp>
#include "MTSQLOracleParserTokenTypes.hpp"
...
class CUSTOM_API MTSQLOracleParser : public ANTLR_USE_NAMESPACE(antlr)LLkParser, public MTSQLOracleParserTokenTypes
{
...
protected:
	MTSQLOracleParser(ANTLR_USE_NAMESPACE(antlr)TokenBuffer& tokenBuf, int k);
public:
	MTSQLOracleParser(ANTLR_USE_NAMESPACE(antlr)TokenBuffer& tokenBuf);
protected:
	MTSQLOracleParser(ANTLR_USE_NAMESPACE(antlr)TokenStream& lexer, int k);
public:
	MTSQLOracleParser(ANTLR_USE_NAMESPACE(antlr)TokenStream& lexer);
	MTSQLOracleParser(const ANTLR_USE_NAMESPACE(antlr)ParserSharedInputState& state);
	int getNumTokens() const
	{
		return MTSQLOracleParser::NUM_TOKENS;
	}
	const char* getTokenName( int type ) const
	{
		if( type > getNumTokens() ) return 0;
		return MTSQLOracleParser::tokenNames[type];
	}
	const char* const* getTokenNames() const
	{
		return MTSQLOracleParser::tokenNames;
	}
	...
	protected: void oracle_lock_statement();
	public: void program();
...
#endif /*INC_MTSQLOracleParser_hpp_*/
