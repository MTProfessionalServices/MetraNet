	...
	BEGIN TRY
		...
		* So, meter tables always have 2 partition.*/
		...
		BEGIN
		    ...
		    /* Update all meter tables set id_partition = @new_current_id_partition if id_sess in ##id_sess_to_keep */
		    IF EXISTS( SELECT TOP(1) * FROM ##id_sess_to_keep)
		    	EXEC archive_queue_partition_move_id_sess_to_keep
		    	     @current_id_partition = @current_id_partition,
		    	     @old_id_partition = @old_id_partition
