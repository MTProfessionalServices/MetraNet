    ...
    for (int i = 0; i < items.Items.Count; i++)
    {
      ...
      if (record.Fields.Count > 0)
      {
        if (operation.Equals("activebillrunsummary"))
        {
          ...
          json.Append(item);
          item = string.Format("\"earliest_eta\":\"{0}\",", string.Format("{0} {1}", Convert.ToDateTime(record.Fields[12].FieldValue).ToString("d"), Convert.ToDateTime(record.Fields[12].FieldValue).ToString("HH:mm:ss")));
          json.Append(item);
          item = string.Format("\"eta_offset\":\"{0}\"", Double.Parse(FormatFieldValue(record.Fields[13])));
          json.Append(item);
        }
      }
      json.Append("}");
