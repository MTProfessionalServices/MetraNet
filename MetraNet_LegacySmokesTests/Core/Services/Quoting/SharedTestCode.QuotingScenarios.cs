  ...
  public class QuotingTestScenarios
  {
    ...
    public static QuoteResponse CreateQuoteAndVerifyResults(QuoteImplementationData quoteImp, QuoteVerifyData expected)
    {
      ...
      finally
      {
        ...
        var cleanupStatus = QuoteStatus.InProgress;
        var exitTimeout = 0;

        while (cleanupStatus == QuoteStatus.InProgress)
        {
          exitTimeout++;
          Thread.Sleep(1000);
          cleanupStatus = quoteImp.QuoteImplementation.QuotingRepository.GetActionStatus(result.IdQuote,
                                                                                         ActionStatus.StatusCleanup);
          if (exitTimeout >= 180) break;
        }

        Assert.AreEqual(cleanupStatus, QuoteStatus.Complete,
                        "Quote cleanup wasn't completed. Failed message: " + result.FailedMessage);
