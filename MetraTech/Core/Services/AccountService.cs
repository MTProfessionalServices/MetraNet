...
namespace MetraTech.Core.Services
{
  ...
  public interface IAccountService
  {
    ...
    void GetAccountList(DateTime metraTimeStamp, ref MTList<Account> accounts, bool displayAliases);

    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void GetAccountListTotalRows(DateTime metraTimeStamp, MTList<Account> accounts, bool displayAliases,
                                 out int totalRows);

    // Load account by id.
    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void LoadAccount(AccountIdentifier acct, DateTime timeStamp, out Account account);

    // Load account by id with all the views.
    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void LoadAccountWithViews(AccountIdentifier acct, DateTime timeStamp, out Account account);

    // Load view using Account Id.
    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void LoadView(AccountIdentifier acct, string viewType, out List<View> views);

    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void MoveAccounts(List<AccountIdentifier> accounts, AccountIdentifier newParent, DateTime timeStamp,
                      DateTime moveStartDate);

    // get the list of account IDs
    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void GetAccountIdList(DateTime metraTimeStamp, ref MTList<Account> accounts, bool displayAliases, out List<int> ids);

    // Add an account without using workflows
    [OperationContract]
    [FaultContract(typeof (MASBasicFaultDetail))]
    void AddAccountWithoutWorkflow(ref Account account);
  ...
  {
    private delegate void ReadAccountData(IMTDataReader reader);

    #region Members

    private static AccountServiceConfigSection _mConfigSection;

    private static IdGenerator _profileIdGenerator;
    private static IdGenerator _accountIdGenerator;

    private static readonly Dictionary<string, IMTPropertyMetaData> MAccountMetaData =
      new Dictionary<string, IMTPropertyMetaData>(StringComparer.CurrentCultureIgnoreCase);
    ...
    {
      var config = LoadConfigurationFile(@"Account\AccountService.xml");
      _mConfigSection = config.GetSection("AccountServiceConfig") as AccountServiceConfigSection;

      using (var accCatalog = new MTComSmartPtr<IMTAccountCatalog>())
      {
        accCatalog.Item = new MTAccountCatalog();
        using (var metaDataSet = new MTComSmartPtr<IMTPropertyMetaDataSet>())
        {
    ...
    public void LoadAccount(AccountIdentifier acct, DateTime timeStamp, out Account account)
    {
    ...
    [OperationCapability("Manage Account Hierarchies")]
    public void GetAccountIdList(DateTime metraTimeStamp, ref MTList<Account> accounts, bool displayAliases,
                                 out List<int> ids)
    {
      // ESR-5877 add GetAccountListTimeOut configuration 
      var getAccountListTimeOut = _mConfigSection.GetAccountListTimeOut;
      using (new HighResolutionTimer("GetAccountIdList", getAccountListTimeOut))
      {
        ...
        var accountCatalog = InitAccountCatalog(clientIdentity);
        //add default filters
        RS.IMTDataFilter filter = new RS.MTDataFilter();
        //Set up columns for return...we actually just need the account id column
        var columns = (YAAC.IMTCollection) new MTCollectionClass();
        var order = (YAAC.IMTCollection) new MTCollectionClass();
        ...
        var strSql = GetAccountListQuery(clientIdentity,
                                         metraTimeStamp,
                                         accounts,
                                         accountCatalog,
                                         ref filter,
          ...
          using (IMTConnection conn = ConnectionManager.CreateConnection())
          {
            GetAccountListInternal(accounts,
                                   ...
                                   delegate(IMTDataReader reader)
                                     {
                                       //resolve account id
                                       var accountId = reader.GetInt32("_AccountID");
    ...
    /// <param name="displayAliases">Indicates which query to use as a base one. If true - "__ADVANCEDSEARCH_ALIASES__", else - "__ADVANCEDSEARCH__"</param>
    [OperationCapability("Manage Account Hierarchies")]
    public void GetAccountList(DateTime metraTimeStamp, ref MTList<Account> accounts, bool displayAliases)
    {
      var getAccountListTimeOut = _mConfigSection.GetAccountListTimeOut;
      using (new HighResolutionTimer("GetAccountList", getAccountListTimeOut))
      {
        // Get the session context from the WCF ambient service security context
        var clientIdentity = ServiceSecurityContext.Current.PrimaryIdentity as CMASClientIdentity;
        if (clientIdentity == null)
        ...
        try
        {
          ...
          using (IMTConnection conn = ConnectionManager.CreateConnection())
          {
            ...
            var accountStringList = new StringBuilder();
            accountList.ForEach(
              delegate(int id) { accountStringList.AppendFormat(accountStringList.Length == 0 ? "{0}" : ",{0}", id); });
            ...
            var accountIdFilter = new MTFilterElement("_AccountID", MTFilterElement.OperationType.In,
                                                      accountStringList.ToString());
            accounts.Filters.Add(accountIdFilter);

            var tmpList = new List<Account>();
            GetAccountListInternal(accounts,
                                   true,
                                   order,
                                   strSql,
                                   ...
                                   delegate(IMTDataReader reader)
                                     {
                                       var accountType = reader.GetString("AccountType");
                                       var accId = reader.GetInt32("_AccountID");
                                       var username = reader.GetString("username");
                                       var nameSpace = reader.GetString("name_space");

                                       var acc =
                                         tmpList.Find(
                                           p =>
                                           (p._AccountID == accId) && (p.UserName == username) &&
                                           (p.Name_Space == nameSpace));

                                       //if not in the list, add the acct to list
                                       if (acc == null)
                                       {
                                         acc = Account.CreateAccount(accountType);
    ...
    [OperationCapability("Manage Account Hierarchies")]
    public void GetAccountListTotalRows(DateTime metraTimeStamp, MTList<Account> accounts, bool displayAliases,
                                        out int totalRows)
    {
      totalRows = -1;
      using (new HighResolutionTimer("GetAccountListTotalRows"))
      {
        try
        {
          // Get the session context from the WCF ambient service security context
          ...
          using (var queryAdapter = new MTComSmartPtr<IMTQueryAdapter>())
          {
    ...
    [OperationCapability("Manage Account Hierarchies")]
    public void LoadAccountWithViews(AccountIdentifier acct, DateTime timeStamp, out Account account)
    {
      using (new HighResolutionTimer("LoadAccountWithViews"))
      {
        var accountId = AccountIdentifierResolver.ResolveAccountIdentifier(acct);
    ...
    [OperationCapability("Manage Account Hierarchies")]
    public void LoadView(AccountIdentifier acct, string viewType, out List<View> views)
    {
      using (new HighResolutionTimer("LoadView"))
      {
        var accountId = AccountIdentifierResolver.ResolveAccountIdentifier(acct);
        views = new List<View>();
    ...
    public void MoveAccounts(
      List<AccountIdentifier> accounts, 
      AccountIdentifier newParent, 
      DateTime timeStamp,
      DateTime moveStartDate)
    {
      #region Validate NewParent
      
      using (new HighResolutionTimer("MoveAccounts"))
      {
        int parentId;
        try
        {
        ...
        var descendents = (Interop.MTYAAC.IMTCollection) new MTCollectionExClass();

        IMTAccountCatalog accountCatalog = new MTAccountCatalogClass();
        accountCatalog.Init((IMTSessionContext) GetSessionContext());

        try
        {
          foreach (var movedAcct in accounts)
          {
        ...
        try
        {
          var actorYaac = accountCatalog.GetAccount(GetSessionContext().AccountID, MetraTime.Now);

          IMTAncestorMgr ancestorMgr = new MTAncestorMgrClass();
          ancestorMgr.Initialize(((IMTSessionContext) GetSessionContext()), actorYaac);
    ...
    [OperationCapability("Manage Account Hierarchies")]
    public void AddAccountWithoutWorkflow(ref Account account)
    {
      using (HighResolutionTimer timer = new HighResolutionTimer("AddAccountWithoutWorkflow"))
      {
        try
        {
          ...
          try
          {
            ...
            if (_profileIdGenerator == null)
            {
              _profileIdGenerator = new IdGenerator("id_profile", 200);
            }
            if (_accountIdGenerator == null)
            {
              _accountIdGenerator = new IdGenerator("id_acc", 200);
            }
            ...
            AccountHelper.CreateAccount(account, sessionContext,
                                        ...
                                        _profileIdGenerator, _accountIdGenerator);
    ...
    public void GetAccountTypeName(long accountId, ref string typeName, ref bool hasLogonCapability)
    {
      try
      {
        ...
        {
          using (
            IMTAdapterStatement stmt = conn.CreateAdapterStatement("queries\\AccHierarchies",
    ...
    private static IMTAccountCatalog InitAccountCatalog(CMASClientIdentity clientIdentity)
    {
      ...
      ReadAccountData readMethod)
    {
      using (var queryAdapter = new MTComSmartPtr<IMTQueryAdapter>())
      {
        ...
        if (null ==
            ((accounts.Filters.Find(f => ((MTFilterElement) f).PropertyName.ToUpper().CompareTo("_ACCOUNTID") == 0))))
        {
        ...
        else
        {
          ...
          {
            queryAdapter.Item.SetQueryTag((displayAliases)
                                            ? "__ADVANCEDSEARCH_ALIASES_WITH_ACCOUNTIDLIST__"
                                            : "__ADVANCEDSEARCH_WITH_ACCOUNTIDLIST__");
          }
        }
        queryAdapter.Item.AddParam("%%INNER_QUERY%%", strSQL, true);
        ...
        var accountIdParam = startIndex;
        var commaSeparatedIdList = new StringBuilder();
          ...
          accounts.Filters.Where(p => ((MTFilterElement) p).Operation == MTFilterElement.OperationType.In))
        {
          ...
          var accountIdList = fe.Value.ToString().Split(new[] {','}).ToList();
          for (var i = 0; i < accountIdList.Count; i++)
          {
        ...
        // Remove trailing ','
        if (commaSeparatedIdList.Length > 0) 
        ...
        var sortColumns = new StringBuilder(2048);

        #region Sort Criteria

        if (accounts.SortCriteria.Count > 0 && !displayAliases)
        {
          foreach (var sc in accounts.SortCriteria)
          {
            object sortOrder = null;
            var columnName = ResolveFilterColumnName(sc.SortProperty, ref sortOrder);
        ...
        if (displayAliases)
        {
          queryAdapter.Item.AddParamIfFound("%%ALLTYPESACCOUNTTYPENAME%%",
                                            AccountTemplateService.Config.AllTypesAccountTypeName, true);
        }

        queryAdapter.Item.AddParam("%%SORT_COLUMN%%", sortColumns.ToString(), true);

        using (var countStmt = conn.CreatePreparedFilterSortStatement(queryAdapter.Item.GetRawSQLQuery(true)))
        {
          ...
          if (accounts.SortCriteria != null && accounts.SortCriteria.Count > 0)
          {
            ...
            {
              object dummy = null;
              var sortColumn = ResolveFilterColumnName(sc.SortProperty, ref dummy);
              var sortDirection = (sc.SortDirection == SortType.Descending) ? " DESC" : "";
          ...
          foreach (MTFilterElement filterElement in
            accounts.Filters.Where(p => ((MTFilterElement) p).Operation == MTFilterElement.OperationType.In))
          {
            var fe = new FilterElement(filterElement.PropertyName.ToUpper(),
                                       (FilterElement.OperationType) ((int) filterElement.Operation),
                                       filterElement.Value);
            ...
            {
              var accountIdList = fe.Value.ToString().Split(new [] {','}).ToList();
              foreach (var str in accountIdList)
              {
          ...
          countStmt.MaxTotalRows = accounts.PageSize*_mConfigSection.GetAccountListMaxPages;
    ...
    protected void RepositionRowsetToAccountID(YAAC.IMTSQLRowset rs, int accountId, string userName, string nameSpace,
                                               bool displayAliases)
    {
    ...
    private void LoadViewInternalFromRS(int accountId, string userName, string nameSpace, string viewType,
                                        YAAC.IMTSQLRowset rs, IList views, bool displayAliases)
    {
    ...
    protected void LoadAccountDataFromRS(int accountId, DateTime timeStamp, BaseTypes.Account acc, YAAC.IMTSQLRowset rs,
                                         bool displayAliases)
    {
      bool allViews = true;
      ...
      foreach (PropertyInfo pi in acc.GetMTProperties())
      {
        ...
        if (IsView(pi, out viewType, out className))
        {
          ...
          object views = CreateGenericObject(typeof (List<>), ass.GetType(acc.GetType().Namespace + "." + className),
                                             null);
    ...
    private void SetValue(DomainModel.BaseTypes.BaseObject obj, PropertyInfo pi, IMTDataReader reader, out object value)
    {
      ...
      {
        mLogger.LogError(String.Format("Couldn't find account property [{0}] in rowset; skipping it. {1}", pi.Name,
                                       ex.Message));
        return;
      }
    ...
    private static object ConverToEnum(PropertyInfo pi, object value)
    {
      ...
      if (strValue != null)
      {
        ...
        if (pi.PropertyType.IsGenericType && pi.PropertyType.GetGenericTypeDefinition().Equals(typeof (Nullable<>)))
        {
          System.ComponentModel.NullableConverter nullableConverter =
            new System.ComponentModel.NullableConverter(pi.PropertyType);
      ...
      else
      {
        ...
        Type enumType = pi.PropertyType.IsEnum
                          ? pi.PropertyType
                          : new System.ComponentModel.NullableConverter(pi.PropertyType).UnderlyingType;
    ...
    private void LoadViewInternalFromReader(int accountId, string userName, string nameSpace, string viewType,
                                            IMTDataReader reader, IList views, bool displayAliases)
    {
      // Populate the passed in list of views.
      try
      {
    ...
    private void LoadAccountDataFromReader(DateTime timeStamp, BaseTypes.Account acc, IMTDataReader reader,
                                           bool displayAliases)
    {
      bool allViews = true;
                                       ...
                                       ref YAAC.IMTCollection columns)
    {
      ...
      filter.Add("AccountStatus", RS.MTOperatorType.OPERATOR_TYPE_NOT_EQUAL, "AR");
      ...
      foreach (
        MTFilterElement filterElement in
          accounts.Filters.Where(p => ((MTFilterElement) p).Operation != MTFilterElement.OperationType.In))
      {
      ...
      //generate the query
      string strSQL = accountCatalog.GenerateParameterizedAccountSearchQuery(metraTimeStamp, columns,
                                                                             (YAAC.IMTDataFilter) filter,
                                                                             null, null, 0);
