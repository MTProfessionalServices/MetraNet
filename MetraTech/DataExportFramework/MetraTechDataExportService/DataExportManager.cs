...
namespace MetraTech.DataExportService
{
  public class DataExportManager : System.ServiceProcess.ServiceBase
  {
    ...
    /// TODO: this and AuditExecuteStatus(ReportInstanceInitializationException riEx) should merge to one method
    private void AuditExecuteStatus(ReportExecutionCompleteEventArgs rE)
    {
      try
      {
        using (IMTConnection auditConnection = ConnectionManager.CreateConnection())
        {
          IMTAdapterStatement selectStat = auditConnection.CreateAdapterStatement(_config.PathToServiceQueryDir, "__AUDIT_REPORT_EXECUTE_STATUS__");
          selectStat.AddParam("%%WORK_QUEUE_ID%%", "'" + rE.WorkQId + "'", true);
          selectStat.AddParam("%%EXECUTE_STATUS%%", "'" + rE.CompletionStatus.ToString() + "'", true);
          selectStat.AddParam("%%EXECUTE_START_DATE_TIME%%", rE.ExecuteStartTime, true);
          selectStat.AddParam("%%EXECUTE_COMPLETE_DATE_TIME%%", rE.DateTimeCompleted, true);
          string description = rE.Description;
          description = description.Replace("%", "*");
          description = description.Replace("'", "");
          description = description.Replace("\"", "");
          selectStat.AddParam("%%DESCR%%", "'" + description + "'", true);
          string paramVals = rE.ExecuteParamValues;
          paramVals = paramVals.Replace("%", "*");
          paramVals = paramVals.Replace("'", "");
          paramVals = paramVals.Replace("\"", "");
          selectStat.AddParam("%%EXECUTE_PARAM_VALUES%%", "'" + paramVals + "'", true);
          DefLog.MakeLogEntry(selectStat.Query, "debug");
          selectStat.ExecuteNonQuery();
    ...
    /// TODO: this and AuditExecuteStatus(ReportExecutionCompleteEventArgs rE) should merge to one method
    private void AuditExecuteStatus(ReportInstanceInitializationException riEx)
    {
      ...
      try
      {
        ...
        {
          IMTAdapterStatement selectStat = auditConnection.CreateAdapterStatement(_config.PathToServiceQueryDir, "__AUDIT_REPORT_EXECUTE_STATUS__");
          selectStat.AddParam("%%WORK_QUEUE_ID%%", "'" + riEx.WorkQId + "'", true);
          selectStat.AddParam("%%EXECUTE_STATUS%%", "'" + riEx.CompletionStatus.ToString() + "'", true);
          selectStat.AddParam("%%EXECUTE_START_DATE_TIME%%", riEx.ExecuteStartTime, true);
          selectStat.AddParam("%%EXECUTE_COMPLETE_DATE_TIME%%", riEx.ExecuteEndTime, true);
          string description = riEx.Descr;
          description = description.Replace("%", "*");
          description = description.Replace("'", "");
          description = description.Replace("\"", "");
          selectStat.AddParam("%%DESCR%%", "'" + description + "'", true);
          string paramVals = riEx.ExecuteParamValues;
          paramVals = paramVals.Replace("%", "*");
          paramVals = paramVals.Replace("'", "");
          paramVals = paramVals.Replace("\"", "");
          selectStat.AddParam("%%EXECUTE_PARAM_VALUES%%", "'" + paramVals + "'", true);
