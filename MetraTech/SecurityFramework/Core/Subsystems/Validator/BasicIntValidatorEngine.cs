...
{
	/// <summary>
	/// Provides a basic validation for 32 bits integer numbers.
	/// </summary>
	internal sealed class BasicIntValidatorEngine : ValidatorEngineBase
	{
		#region Constants

		private const string InputNonIntegerValue = "Input string does not represent an integer.";
		private const string UnableConvertInputValue = "Unable to convert an input value to integer number.";
		...
		/// </summary>
		/// <remarks>Always returns <see cref="Int32"/>.</remarks>
		...
		/// <summary>
		/// Creats an instance of the <see cref="BasicIntValidatorEngine"/> class and initializes its properties.
		/// </summary>
		public BasicIntValidatorEngine()
		...
		protected override ApiOutput ValidateInternal(ApiInput input)
		{
			...
			int var = ConvertToInt(inputStr);

			if (var < Min || var > Max)
			{
				throw new ValidatorInputDataException(
					Common.ExceptionId.Validator.InputIntegerNotInRange, Category, InputNotInRange, inputStr, string.Format(NotInRangeReasonFormat, Min, Max));
			}
		...
		private int ConvertToInt(string inputStr)
		{
			...
			if (ExpectHexFormat)
			{
				try
				{
					...
					var = int.Parse(inputStr, NumberStyles.HexNumber, CultureInfo.CurrentCulture);
				}
				catch (Exception ex)
				{
					throw new ValidatorInputDataException(
			...
			else if (DoFormatAutoDetect)
			{
				try
				{
					...
					{
						inputStr = inputStr.Substring(2);
						var = Convert.ToInt32(inputStr, 16);
					}
					else if (inputStr.Length >= 2 && inputStr.StartsWith(Constants.Validation.OctalNumberPrefix))
					{
						inputStr = inputStr.Substring(2);
						var = Convert.ToInt32(inputStr, 8);
					}
					else
					{
						var = Convert.ToInt32(inputStr, 10);
					}
				}
				catch (Exception ex)
				{
			...
			else if (!int.TryParse(inputStr, out var))
			{
				throw new ValidatorInputDataException(
					Common.ExceptionId.Validator.InputValueIsNotInteger, Category, InputNonIntegerValue, inputStr, UnableConvertInputValue);
			}
