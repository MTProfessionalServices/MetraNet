...
namespace MetraTech.Tax.Plugins
{
    ...
    public sealed class GeneralConfig
    {
        ...
        /// <summary>
	/// The value in this field identifies the type of data in the 
	/// OriginLocation field and tells CTQ how to access the data to 
	/// find the origination jurisdiction. The values are: 
	/// G = GeoCode, identifies the value in OriginLocation as a GeoCode, 
	/// N = NPA/NXX, identifies the value in OriginLocation as an NPA/NXX, 
	/// P = ZIP + 4 Code, identifies the value in OriginLocation as a ZIP + 4 Code.
        /// </summary>
        ...
        /// <summary>
	/// The value in this field identifies the type of data in the 
	/// TerminationLocation field and tells CTQ how to access the data to 
	/// find the termination jurisdiction. The values are: 
	/// G = GeoCode, identifies the value in TerminationLocation as a GeoCode, 
	/// N = NPA/NXX, identifies the value in TerminationLocation as an NPA/NXX, 
	/// P = ZIP + 4 Code, identifies the value in TerminationLocation as a ZIP + 4 Code.
        /// </summary>
    ...
    public sealed class PipelineProperties
    {
       ...
       private Binding<decimal> m_othertaxamountrounded;
        ...
        private PipelineProperties(PlugInBase.LogDelegate log, IMTSystemContext systemContext, XmlDocument xmlConfig)
        {
            ...
            m_originlocationmode = CreateOptionalBindingString(nameID, GetValueFromParameter(pipelineProps, "OriginLocationMode"));
            m_terminationlocationmode = CreateOptionalBindingString(nameID, GetValueFromParameter(pipelineProps, "TerminationLocationMode"));
        ...
        /// <summary>
        /// (In) This account will be examined to determine country and country zone.  This will then be used with parameter tables containing VertexQ rates and other information to determine the tax rate.
        /// </summary>
        ...
        /// <summary>
        /// (In) This is the date of the transaction.  The date is used to select the best fitting rate from the VertexQ parameter tables.
        /// </summary>
        ...
        {        
            get
            {
                try
                {
                    if (m_categorycode.ID == -1)
                    {
                        m_categorycode.IsNull = true;
                    }
                    else if (!m_categorycode.HasValue)
                    {
        ...
        {        
            get
            {
                try
                {
                    if (m_servicecode.ID == -1)
                    {
                        m_servicecode.IsNull = true;
                    }
                    else if (!m_servicecode.HasValue)
                    {
        ...
        /// <summary>
        /// (In) A code identifying the product.  This same code should be used in the VertexQ parameter table define the tax rate for the product.
        /// </summary>
        ...
        {        
            get
            {
                try
                {
                    if (m_originlocationmode.ID == -1)
                    {
                        m_originlocationmode.IsNull = true;
                    }
                    else if (!m_originlocationmode.HasValue)
                    {
        ...
        {        
            get
            {
                try
                {
                    if (m_terminationlocationmode.ID == -1)
                    {
                        m_terminationlocationmode.IsNull = true;
                    }
                    else if (!m_terminationlocationmode.HasValue)
                    {
