...
-(void)refresh {
	...
	// add button
	addButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addMethod:)];
    self.navigationItem.rightBarButtonItem = addButton;
	
	// load data
	dataSourceArray = [[((MetraViewXcodeAppDelegate *)[UIApplication sharedApplication].delegate).mv GetSubscriptions] retain];
...
- (void)viewDidLoad {
	...
	self.title = NSLocalizedString(@"Subscriptions", @"");		
...
-(void)addMethod:(id)sender {
	...
	AvailableSubscriptionsViewController *availableSubscriptionsViewController = [[AvailableSubscriptionsViewController alloc] initWithNibName:@"AvailableSubscriptionsView" bundle:nil];
	[self.navigationController pushViewController:availableSubscriptionsViewController animated:YES];
	[availableSubscriptionsViewController release];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	cell.detailTextLabel.text = [NSString stringWithFormat:@"%@  %@", [item objectForKey:@"description"], [item objectForKey:@"date"]];
