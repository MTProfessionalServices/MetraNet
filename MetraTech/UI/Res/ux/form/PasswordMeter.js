...
Ext.ux.PasswordMeter = function (config) {
    ...
    if(config.cls)
        this.objMeter.addClass(config.cls);
};
...
Ext.extend(Ext.ux.PasswordMeter, Ext.form.TextField, {
    ...
    */
    ...
    onRender: function (ct, position) {
        ...
        this.objMeter.setWidth(elp.getWidth(true) -3 );
    ...
    */
    calcStrength: function (p) {
        ...
        if (p.length > 0 && p.length <= 3) {         // length 3 or less
            intScore += p.length;
        }
        else if (p.length >= 4 && p.length <= 6) {	// length between 4 and 6
            intScore += 6;
        }
        else if (p.length >= 7 && p.length <= 15) {	// length between 7 and 15
            intScore += 12;
            //alert(intScore);
        }
