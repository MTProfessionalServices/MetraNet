...
Ext.extend(Ext.ux.grid.GridFilters, Ext.util.Observable, {
	...
	onRender: function(){
		...
		this.menu = hmenu.add(new Ext.menu.CheckItem({
			...
			*/
	  this.menu = new Ext.menu.CheckItem();
	...
	updateColumnHeadings: function(view){
		...
		for(var i=0, len=view.cm.config.length; i<len; i++){
		  var di = view.cm.config[i].dataIndex;
		  
		  if(di.endsWith('ValueDisplayName'))
		  {
		    di = di.replace(/ValueDisplayName/, '');
		  }
		  
			var filter = this.getFilter(di);
	...
	buildQuery: function(filters){
		...
		for(var i=0, len=filters.length; i<len; i++){
			...
			for(var key in f.data)
				p[[dataPrefix, '[', key, ']'].join('')] = Ext.util.Format.htmlEncode(f.data[key]);
		}
