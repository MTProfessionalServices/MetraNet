...
namespace MetraTech.UsageServer
{
  ...
  [Guid("7F78C83F-82E2-3B84-A247-E7F77745C74A")]
	...
	public class Cycle : ICycle
	{
		...
		public int DayOfMonth
		{
			...
			set
			{
				if (!(value > 0 && value <= MAX_DAYS_IN_MONTH))
					throw new UsageServerException(String.Format("Day of month value {0} is not between 1 and {1}", value,MAX_DAYS_IN_MONTH), true);
				mDayOfMonth = value;
			}
		}
        ...
        if (!(value >= 1 && value <= MAX_DAYS_IN_MONTH))
					throw new UsageServerException(String.Format("StartDay {0} must be between 1 and {1}", value,MAX_DAYS_IN_MONTH), true);
		...
		public static DateTime LastDayOfMonth(DateTime date)
    {
		...
		public static DateTime MoveToDay(DateTime date, int dayOfMonth)
		{
            ...
            DateTime movedDate = new DateTime(date.Year, date.Month, targetDay);
		...
		{
			if (String.Compare(cycleType, "Monthly", true) == 0)
				return CycleType.Monthly;
			if (String.Compare(cycleType, "Daily", true) == 0)
				return CycleType.Daily;
			if (String.Compare(cycleType, "Weekly", true) == 0)
				return CycleType.Weekly;
			if (String.Compare(cycleType, "Bi-weekly", true) == 0)
				return CycleType.BiWeekly;
			if (String.Compare(cycleType, "Semi-monthly", true) == 0)
				return CycleType.SemiMonthly;
			if (String.Compare(cycleType, "Quarterly", true) == 0)
				return CycleType.Quarterly;
			if (String.Compare(cycleType, "Annually", true) == 0)
				return CycleType.Annual;
      if (String.Compare(cycleType, "Semi-Annually", true) == 0)
        return CycleType.SemiAnnual;
      if (String.Compare(cycleType, "All", true) == 0)
				return CycleType.All;

			throw new System.ArgumentException(String.Format("Invalid cycle type name {0}", cycleType));
		}
	...
	public class QuarterlyCycleType : ICycleType
	{
																			 ...
																			 out DateTime endDate)
		{
			...
			quarterStarts[1] = new DateTime(today.Year, cycle.StartMonth, Math.Min(cycle.StartDay, DateTime.DaysInMonth(today.Year, cycle.StartMonth)));
	...
	public class BiWeeklyCycleType : ICycleType
	{
		...
		public void MakeCanonical(ICycle cycle)
		{
			...
			if (!(day <= daysInMonth))
				throw new UsageServerException(String.Format("StartDay {0} must be a valid day of month for the Bi-weekly cycle", day), true);
