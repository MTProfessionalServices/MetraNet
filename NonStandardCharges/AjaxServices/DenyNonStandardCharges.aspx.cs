...
public partial class NonStandardCharges_AjaxServices_DenyNonStandardCharges : MTListServicePage
{
  protected void Page_Load(object sender, EventArgs e)
  {
    var response = new AjaxResponse();
    using (new HighResolutionTimer("DenyNonStandardCharges", 5000))
    {
      ...
      try
      {
        ...
        client.DenyNonStandardCharges(sessionIds); ;
        response.Success = true;        
        var message = GetGlobalResourceObject("Adjustments", "TEXT_Successfully_denied_nonstandard_charges");
        if (message != null)
