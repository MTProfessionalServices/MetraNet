...
public partial class NonStandardCharges_AjaxServices_LoadCharges : MTListServicePage
{
  private const int MAX_RECORDS_PER_BATCH = 50;

  protected bool ExtractDataInternal(NonStandardChargeServiceClient client, ref MTList<Metratech_com_NonStandardChargeProductView> items, int batchID, int limit)
  {
    try
    {
      items.Items.Clear();
      ...
      client.GetNonStandardCharges(ref items);
    }
  ...
  protected bool ExtractData(NonStandardChargeServiceClient client, ref MTList<Metratech_com_NonStandardChargeProductView> items)
  {
  ...
  {
    using (new HighResolutionTimer("GetNonStandardCharges", 5000))
    {
      ...
      try
      {
        ...
        var items = new MTList<Metratech_com_NonStandardChargeProductView>();
        items.Filters.Add(new MTFilterElement("Status", MTFilterElement.OperationType.Equal, "P"));
        SetPaging(items);
        SetSorting(items);
        SetFilters(items);
        ...
        if (Page.Request["mode"] != "csv")
        {
          //convert non standard charges into JSON
