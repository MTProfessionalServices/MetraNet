...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
  ...
  <MT:MTTitle ID="MTTitle1" Text="Add Partition" runat="server" meta:resourcekey="MTTitle1Resource1" /><br />
      ...
      <table cellspacing="0">
        <tr>
          <td  class="x-panel-btn-td">
            <MT:MTButton ID="btnOK" OnClientClick="return Validate();" Width="50px" runat="server" Text="<%$Resources:Resource,TEXT_OK%>" OnClick="btnOK_Click" TabIndex="390" />
          </td>
          <td  class="x-panel-btn-td">
  ...
  <script type="text/javascript" language="javascript">
    function Validate() {
      var username = window.Ext.get("<%= tbUserName.ClientID%>").dom.value;
      var passwd = window.Ext.get("<%= tbPassword.ClientID%>").dom.value;
      var confpasswd = window.Ext.get("<%= tbConfirmPassword.ClientID%>").dom.value;
      var authType = window.Ext.get("<%= ddAuthenticationType.ClientID%>").dom.value;

      if ((authType == '<%=AuthenticationType.MetraNetInternal.ToString()%>') && ((passwd == '') || (confpasswd == '')) || (username == '')) {
        window.Ext.Msg.show({
          title: window.TEXT_ERROR,
          msg: window.TEXT_VALIDATION_ERROR,
        ...
        window.Ext.Msg.show({
          title: window.TEXT_ERROR,
          msg: window.TEXT_PASSWORDS_DO_NOT_MATCH,
          buttons: window.Ext.Msg.OK,
          icon: window.Ext.MessageBox.ERROR
        });
        return false;
