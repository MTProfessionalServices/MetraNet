...
my $cacheDir = "c:\\scratch\\rscache";

#
# initialize a rule set
#
sub GenerateRateSchedule($)
{
	my ($id) = @_;
	...
	$action->{PropertyName} = "RuleSetID";
	$action->{PropertyValue} = $id;
	$action->{PropertyType} = $PROP_TYPE_INTEGER;
	$actions->Add($action);
	...
	my $filename = "$cacheDir\\Schedule$id.xml";
	$ruleset->Write($filename);
}
