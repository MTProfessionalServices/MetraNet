...
<!--      <td align="right" bgcolor="#000099"><font face="Arial" color="white" size="1"><b>Incurred Charges</b></font></td> -->
  ...
  <!-- Apply templates for the charges -->
    <xsl:apply-templates select="charge[display='Derek Young']">
      <xsl:sort select="display" />
    </xsl:apply-templates>
    
    <xsl:apply-templates select="charge_group[.//charge[display='Derek Young']]">
      <xsl:sort select="display" />
    </xsl:apply-templates>
  <!-- End Templates -->
  </table>
    ...
    <td align="left">
      ...
      <nobr>
        <font face="Arial" size="1" color="navy">
          <a href="#"><img border="0" src="/samplesite/us/images/icons/genericProduct.gif" />
  ...
  <tr>
    <td align="left">
      ...
      <nobr>
        <font face="Arial" size="1" color="navy">
          <xsl:if test="string-length(amount) != 0">
            <a href="#"><img border="0" src="/samplesite/us/images/icons/genericProduct.gif" /></a>
          </xsl:if>
    ...
    <td style="padding-top:3px;" align="right">
      <xsl:if test="string-length(charge/amount) != 0">    
        <nobr>
          <font face="Arial" size="1" color="navy">$</font>
          <font face="Courier New" style="font-size:8pt;" color="navy"><xsl:value-of select="msxsl:formatNumber(./charge/amount, '###,###,##0.00')" /></font>
        </nobr>
      </xsl:if>
    </td>
