<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/account_hierarchies/hierarchy">
        ...
        <td>
          <table cellspacing="0" cellpadding="0" border="0">
            <tr valign="middle">
              ...
              <td class="clsMenuRootText"><xsl:value-of select="child" /></td>
            </tr>
          </table>
        </td>
      </tr>
  ...
  <xsl:template match="hierarchy">
    <table cellspacing="0" cellpadding="0" border="0">
    <xsl:attribute name="id">shadow<xsl:value-of select="id_acc" /></xsl:attribute>
      <tr valign="middle">
        <!-- If a parent, draw the + and - boxes -->
        <xsl:if test="mt:IsParent(.) = 1">
...
<!--              <xsl:attribute name="id"><xsl:value-of select="id" /></xsl:attribute> -->
          ...
          <td nowrap="true">
            ...
            <xsl:attribute name="onClick"><xsl:value-of select="mt:GetClickEvent(.)" /></xsl:attribute>
            <xsl:attribute name="dragID"><xsl:value-of select="id_acc" /></xsl:attribute>
            <xsl:attribute name="onMouseDown">SelectMe('<xsl:value-of select="id_acc" />');</xsl:attribute>
            <xsl:attribute name="onMouseOver">window.status='<xsl:value-of select="mt:GetFQN(.)" />';</xsl:attribute>
            <xsl:value-of select="child" />
            <xsl:if test="@effdate">
              <xsl:text>  </xsl:text>(<xsl:value-of select="@effdate" />)
            </xsl:if></td>
        </xsl:if>
        <xsl:if test="mt:IsParent(.) = 0">
          <td class="clsMenuGroupText" nowrap="true" width="1%">
            <img>
  ...
  <![CDATA[
    ...
    function GetFQN(objTmp) {
      ...
      if(objNode.parentNode == null)
         return(objNode.selectSingleNode('child').text);

      if(objNode.parentNode.nodeName == 'hierarchy')
        strFQN = GetFQN(objNode.parentNode.selectNodes('.')) + '/' + objNode.selectSingleNode('child').text; 
      else 
        return('/' + objNode.selectSingleNode('child').text);
    ...
    function IsParent(objTmp) {
      ...
      objAttr = objNode.attributes.getNamedItem('bChildren');
      
      if(objAttr == null)
        return(0);
        
      if(objAttr.value.toUpperCase() == "Y")
        return(1);
      else
        return(0);
    }
    ...
    function GetMenuConnectorImage(objTmp, intPos) {
      ...
      if(intPos == objNode.parentNode.selectNodes('hierarchy').length)
        bLast = true;
      else
        bLast = false;
    ...
    function GetClickEvent(objTmp) {
      ...
      objIDNode = objNode.selectSingleNode('id_acc');
    ...
    function GetClass(objTmp) {
      ...
      //Check for disabled
      objAttr = objNode.attributes.getNamedItem('effdate');
      
      if(objAttr != null)
        return('clsDragDisabled');
