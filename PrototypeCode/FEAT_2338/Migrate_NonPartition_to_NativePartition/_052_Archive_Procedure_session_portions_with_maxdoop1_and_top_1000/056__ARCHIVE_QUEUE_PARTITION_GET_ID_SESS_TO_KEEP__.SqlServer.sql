IF OBJECT_ID('archive_queue_partition_get_id_sess_to_keep') IS NOT NULL
    DROP PROCEDURE archive_queue_partition_get_id_sess_to_keep
GO
...
AS
	...
	WHERE  st.id_partition = @old_id_partition
	       AND tx_state IN ('F', 'R')
	       AND dt_end = @max_time
	OPTION(MAXDOP 1)
	...
	FROM   t_session sess
	WHERE  sess.id_partition = @old_id_partition
	       AND NOT EXISTS (
	               SELECT 1
	               FROM   t_session_state st
	               WHERE  st.id_partition = @old_id_partition
	...
	WHERE  ts.id_partition = @old_id_partition
