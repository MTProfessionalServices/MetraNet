...
namespace BaselineGUI
{
    ...
    public class ProductView
    {
        ...
        public void loadFromDB(int id)
        {
            ...
            List<ProdViewProp> props = dc.ExecuteQuery<ProdViewProp>(string.Format("select nm_name, nm_column_name from t_prod_view_prop where id_prod_view={0}", id)).ToList<ProdViewProp>();
            foreach (ProdViewProp prop in props)
            {
                // Product view in the DB has more properties than the XML file
                if (dict.ContainsKey(prop.nm_name))
                {
        ...
        {
            //BaselineManager bm = Framework.instance.bm;
            //liveStatus = new LiveStatus(bm, bm.TextBoxSvcDef, bm.LabelSvcDef);
            ...
            foreach (XmlElement elt in nodes)
            {
                ...
                if (field.theType == "enum")
                {
                    ...
                    //E(string.Format("   enum {0} {1}", field.enumSpace, field.enumType));
                }


                //E(string.Format("{0} {1} length:{2} default:{3}", field.name, field.theType, field.length, field.defaultValue));
                //E(field.description);
                fields.Add(field);
