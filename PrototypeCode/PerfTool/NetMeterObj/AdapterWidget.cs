...
namespace NetMeterObj
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(AdapterWidget));
        ...
        {
            log.InfoFormat(">>Build {0}", tableName);
            //this.tableName = tableName;
            this.conn = conn;

            log.DebugFormat("getObjectID for {0}", tableName);
            getObjectID();
            ...
            log.DebugFormat("executing {0}", Sql);
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandText = Sql;
            cmd.CommandType = CommandType.Text;
            dataAdapter.SelectCommand = cmd;
            ...
            log.Debug(string.Format("getForeignKeys for {0}", tableName));
            getForeignKeys();

            log.Debug(string.Format("buildInsertCmds for {0}", tableName));
            buildInsertCmds();
            log.Debug(string.Format("<< build for {0}", tableName));
        }
