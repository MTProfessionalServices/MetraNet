...
namespace NetMeterObj
{
    public class IdAllocatorBasic
    {
        ...
        public bool refreshFromDB()
        {
            ...
            {
                if (tableName == "t_current_id")
                {
                    sql = string.Format("select {0} as value, id_min_id from {1} where nm_current='{2}'", valColumn, tableName, key);
                }
                else
                {
