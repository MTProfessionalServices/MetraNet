...
/*				foreign key(id_descendent) references t_account(id_acc)                                             */
        ...
        alter table t_counter_param_map add constraint fk_param_map_counter_params_metadata
        foreign key(id_counter_param_meta) references t_counter_params_metadata(id_prop)
				...
				alter table T_PIPELINE_SERVICE add  constraint FK1_T_PIPELINE_SERVICE
				foreign key (ID_PIPELINE) references T_PIPELINE (ID_PIPELINE) 

				alter table T_PIPELINE_SERVICE add  constraint FK2_T_PIPELINE_SERVICE
				foreign key (ID_SVC) references T_ENUM_DATA (ID_ENUM_DATA) 
	...
	-- t_adjustment_transaction
	
				-- Temporary removal to support partitioning prototype 
				--alter table t_adjustment_transaction add constraint t_adjustment_trx_fk1
				--foreign key (id_sess) references t_acc_usage(id_sess)
...
-- This FK is necessary, but right now it preventing subscriptions from being properly updated
-- so we will comment it out for now
--		alter table t_recur_value
--			add constraint t_recur_value_fk2 foreign key (id_sub) references t_sub(id_sub) 
				...
				-- foreign keys for partitioning metadta
				--
