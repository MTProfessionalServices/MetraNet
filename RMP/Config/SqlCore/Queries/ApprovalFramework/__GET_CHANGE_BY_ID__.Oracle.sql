			...
			c_Comment, c_CurrentState, de.tx_desc as CurrentStateDisplayName, c_UniqueItemId, c_ChangeDetails 
			FROM  t_approvals a
      ...
      LEFT JOIN (select * from t_description inner join t_language on tx_lang_code = '%%id_languagecode%%' and t_description.id_lang_code = t_language.id_lang_code) d 
            ON e.id_enum_data = d.id_desc      		
			LEFT JOIN (select * from t_description inner join t_language on tx_lang_code = '%%id_languagecode%%' and t_description.id_lang_code = t_language.id_lang_code) de 
