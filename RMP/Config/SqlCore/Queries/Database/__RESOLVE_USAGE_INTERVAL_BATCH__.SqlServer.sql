...
SELECT 
  ...
  results.id_interval,
  arg.id_acc 
FROM %%TABLE_NAME%% arg WITH(READCOMMITTED)
LEFT OUTER JOIN 
(
  SELECT 
    arg2.id_request,
    ui.id_interval 
  FROM %%TABLE_NAME%% arg2 WITH(READCOMMITTED)
  ...
  WHERE
    ...
    -- normalizes the start date of the interval to the effectivity of the mapping if one exists
	  arg2.dt_session BETWEEN ISNULL(dbo.AddSecond(aui.dt_effective), ui.dt_start) AND ui.dt_end
