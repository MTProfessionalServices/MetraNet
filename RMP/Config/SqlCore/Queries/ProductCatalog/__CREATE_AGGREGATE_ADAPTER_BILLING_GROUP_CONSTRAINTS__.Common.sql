...
/*  */
/*  Creates Aggregate Rating constraint groups */
/*  */
/*  For a given set of payers, the query will check all payees of */
/*  those payers during the interval for subscriptions to POs containing aggregate PIs. */
/*  If any payees are found to be affiliated with a aggregate PI, all payers (on the same */
/*  billing cycle as the interval) of that account and any related accounts (siblings in a */
/*  gsub) for the interval will be added to a constraint group. */
/*  */
/*  NOTE: this constraint applies even if an aggregate interval doesn't end in the current */
/*  billing interval. This *IS* necessary because aggregate results are emitted incrementally */
/*  due to the tapered-ness of the underlying counter calculations. */
/*  */
begin
...
where exists
	...
	and exists
		...
		select 1 from t_pl_map typemap 
		  INNER JOIN t_aggregate agg ON agg.id_prop = typemap.id_pi_template
