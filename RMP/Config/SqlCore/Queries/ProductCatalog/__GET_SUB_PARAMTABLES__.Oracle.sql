				...
				case when tb_ip.id_prop is NULL then tb.nm_display_name else
				(tb_ip.nm_display_name || '/' || tb.nm_display_name) end instance_nm_name,
				tb_pt.nm_name pt_nm_name,
				tb_po.nm_name po_nm_name,
				tb_po.nm_display_name po_nm_display_name,
				case when (select count(id_sub) from t_pl_map map2 where map2.id_pi_instance = map.id_pi_instance AND
				map2.id_paramtable = map.id_paramtable and map2.id_sub = sub.id_sub and map2.id_acc = %%ID_ACC%%) > 0 then 'Y' else 'N' end b_PersonalRate,
				map.b_canICB,
