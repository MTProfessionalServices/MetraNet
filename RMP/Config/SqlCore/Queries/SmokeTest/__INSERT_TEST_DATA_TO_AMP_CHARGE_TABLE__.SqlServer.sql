		...
		BEGIN
			/* Append 'agg_charge_audit_trail' with test data in different partitions */
			SELECT @new_id_sess = ISNULL(MAX(id_sess), 0) + 1 FROM agg_charge_audit_trail			
			INSERT INTO agg_charge_audit_trail
			(
				id_payee, id_sess, id_usage_interval				
			)
			VALUES
			(
				1, @new_id_sess, @id_interval_start
			)
