             ...
             begin
              ...
              v_tx_id := mt_ttt.get_tx_id();	   
		          ...
		          insert into ttt_tmp_session (tx_id, id_ss, id_source_sess) 
		              select v_tx_id, mod(id_sess, %%NUM_ATOMIC_MESSAGES%%)+ %%ID_SESSIONSET_START%%, id_parent_source_sess 
			            from ttt_tmp_aggregate
                  where tx_id = v_tx_id;
    			        
		          insert into ttt_tmp_session_set(tx_id, id_ss, id_message, id_svc, session_count, b_root)
			            select v_tx_id, id_ss, mod(id_ss, %%NUM_ATOMIC_MESSAGES%%) + %%ID_MESSAGE_START%%,%%ID_SVC%%, count(*), '1'
			            from ttt_tmp_session
			            where (id_ss >= %%ID_SESSIONSET_START%%) and (id_ss < (%%ID_SESSIONSET_START%%+%%NUM_ATOMIC_MESSAGES%%))
                    and tx_id = v_tx_id
			            group by tx_id, id_ss;
    			         
			        insert into ttt_tmp_message(tx_id, id_message)
			            select v_tx_id, id_message from ttt_tmp_session_set
			            where (id_message >= %%ID_MESSAGE_START%%) and (id_message < (%%ID_MESSAGE_START%%+%%NUM_ATOMIC_MESSAGES%%))
                    and tx_id = v_tx_id;
