 ...
 begin
    ...
    inner join ttt_tmp_session ss
    on ss.tx_id = v_tx_id and ss.id_source_sess = rr.id_parent_source_sess
    ...
    order by ss.tx_id, ss.id_ss, ss.id_source_sess;
 	...
 	insert into ttt_tmp_session_set (tx_id, id_ss, id_message, id_svc, session_count, b_root)
      ...
      on ss.tx_id = v_tx_id and ss.id_source_sess = rr.id_source_sess
      where ss.tx_id = v_tx_id
      and ss.id_ss >= (%%ID_SESSIONSET_START%% + (%%NUM_REGULAR_MESSAGES%% * %%I%%))
      and ss.id_ss < (%%ID_SESSIONSET_START%% + (%%NUM_REGULAR_MESSAGES%% * (%%I%% + 1)))
      group by ss.tx_id, ss.id_ss
      order by ss.tx_id, ss.id_ss, min(rr.id_parent_source_sess)) tmpss
 		inner join
      (select parent.tx_id, parent.id_ss,
              parent.id_source_sess, parentset.id_message
