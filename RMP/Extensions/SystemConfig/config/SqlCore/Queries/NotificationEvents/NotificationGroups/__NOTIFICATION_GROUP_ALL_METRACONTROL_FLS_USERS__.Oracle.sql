...
/* accounts that have the Application Logon to MOM and View/Manage FLS Files capability */    
SELECT 		
	prp.id_acc 
...
WHERE 1=1 
  ...
  -- And this account also has View and/or Manage FLS Files capability
  AND EXISTS (	SELECT DISTINCT prp_fls.id_acc
	 FROM
     T_PRINCIPAL_POLICY prp_fls
     ...
     WHERE  
      prp_fls.id_acc = prp.id_acc and
      prp_fls.policy_type = 'A'
	 and cct.tx_name in ('View FLS Files','Manage FLS Files') ) 

UNION ALL

/* accounts that are assigned a role which includes Application Logon to MOM and View/Manage FLS Files capability */
SELECT 		
	prp.id_acc  
	,acc.id_acc AS id_Partition
FROM     
...
WHERE  1=1 
  AND prp.id_acc IS NOT NULL 
  AND prp.policy_type = 'A' 
  AND pr.id_role = (SELECT 
						CASE 
						WHEN CAPABILITIES.CAPBILITIESCOUNT >= 2 THEN pr.id_role
						ELSE NULL
						END
					FROM (SELECT 
							count(1) AS CAPBILITIESCOUNT
							, SUBR.id_role 
						  FROM T_ROLE SUBR
						  ...
						  WHERE ed.nm_enum_data = 'Global/Application/MOM' OR ct.tx_name IN ('View FLS Files','Manage FLS Files')
...
WHERE  1=1 
  ...
  AND EXISTS (SELECT DISTINCT prp_fls.id_acc
          ...
          INNER JOIN t_capability_instance ci ON prp_fls.ID_POLICY = CI.ID_POLICY
          LEFT OUTER JOIN t_composite_capability_type cct ON ci.id_cap_type = cct.id_cap_type
          WHERE prp_fls.id_acc = prp.id_acc AND prp_fls.policy_type = 'A' AND cct.tx_name IN ('View FLS Files','Manage FLS Files')) 
  ...
  AND pr.id_role IN (SELECT SUBR.id_role
					FROM T_ROLE SUBR
					    INNER JOIN T_PRINCIPAL_POLICY RPP ON SUBR.ID_ROLE = RPP.ID_ROLE
						  INNER JOIN t_capability_instance ci ON RPP.ID_POLICY = CI.ID_POLICY
						  LEFT OUTER JOIN t_Composite_capability_type ct ON ci.id_cap_type = ct.id_cap_type						  
						  WHERE SUBR.id_role = pr.id_role AND ct.tx_name IN ('View FLS Files', 'Manage FLS Files'))
