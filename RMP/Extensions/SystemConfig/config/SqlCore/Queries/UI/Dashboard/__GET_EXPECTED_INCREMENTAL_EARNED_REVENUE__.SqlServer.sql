...
SELECT 
	...
	,c_ProratedIntervalEnd = CASE
								...
								END
	,c_ProratedDays = CASE 
					WHEN udrc.c_RCIntervalSubscriptionStart <= %%START_DATE%%
						AND udrc.c_RCIntervalSubscriptionEnd >= @endDate
					...
					WHEN udrc.c_RCIntervalSubscriptionStart >= %%START_DATE%%
						AND udrc.c_RCIntervalSubscriptionStart <= @endDate
						AND udrc.c_RCIntervalSubscriptionEnd >= @endDate
						THEN DATEDIFF(DAY, udrc.c_RCIntervalSubscriptionStart, @endDate)
					WHEN udrc.c_RCIntervalSubscriptionStart <= %%START_DATE%%
						AND udrc.c_RCIntervalSubscriptionEnd >= %%START_DATE%%
						AND udrc.c_RCIntervalSubscriptionEnd <= @endDate
						THEN DATEDIFF(DAY, %%START_DATE%%, udrc.c_RCIntervalSubscriptionEnd)
					WHEN frc.c_RCIntervalSubscriptionStart <= %%START_DATE%%
						AND frc.c_RCIntervalSubscriptionEnd >= @endDate
						THEN DATEDIFF(DAY, %%START_DATE%%, @endDate)
					WHEN frc.c_RCIntervalSubscriptionStart >= %%START_DATE%%
						AND frc.c_RCIntervalSubscriptionEnd <= @endDate
						THEN DATEDIFF(DAY, frc.c_RCIntervalSubscriptionStart, frc.c_RCIntervalSubscriptionEnd)+1
					WHEN frc.c_RCIntervalSubscriptionStart >= %%START_DATE%%
						AND frc.c_RCIntervalSubscriptionStart <= @endDate
						AND frc.c_RCIntervalSubscriptionEnd >= @endDate
						THEN DATEDIFF(DAY, frc.c_RCIntervalSubscriptionStart, @endDate)
					WHEN frc.c_RCIntervalSubscriptionStart <= %%START_DATE%%
						AND frc.c_RCIntervalSubscriptionEnd >= %%START_DATE%%
						AND frc.c_RCIntervalSubscriptionEnd <= @endDate
						THEN DATEDIFF(DAY, %%START_DATE%%, frc.c_RCIntervalSubscriptionEnd)
					WHEN nrc.c_NRCIntervalSubscriptionStart IS NOT NULL
						THEN 1
...
WHERE
	(
		-- UDRC conditions
		((udrc.c_RCIntervalSubscriptionStart <= %%START_DATE%% AND udrc.c_RCIntervalSubscriptionEnd >= @endDate)
			OR (udrc.c_RCIntervalSubscriptionStart >= %%START_DATE%% AND udrc.c_RCIntervalSubscriptionEnd <= @endDate)
			OR (udrc.c_RCIntervalSubscriptionStart >= %%START_DATE%% AND udrc.c_RCIntervalSubscriptionStart <= @endDate AND udrc.c_RCIntervalSubscriptionEnd >= @endDate)
			OR (udrc.c_RCIntervalSubscriptionStart <= %%START_DATE%% AND udrc.c_RCIntervalSubscriptionEnd >= %%START_DATE%% AND udrc.c_RCIntervalSubscriptionEnd <= @endDate))
	-- FRC conditions
		OR ((frc.c_RCIntervalSubscriptionStart <= %%START_DATE%% AND frc.c_RCIntervalSubscriptionEnd >= @endDate)
			OR (frc.c_RCIntervalSubscriptionStart >= %%START_DATE%% AND frc.c_RCIntervalSubscriptionEnd <= @endDate)
			OR (frc.c_RCIntervalSubscriptionStart >= %%START_DATE%% AND frc.c_RCIntervalSubscriptionStart <= @endDate AND frc.c_RCIntervalSubscriptionEnd >= @endDate)
			OR (frc.c_RCIntervalSubscriptionStart <= %%START_DATE%% AND frc.c_RCIntervalSubscriptionEnd >= %%START_DATE%% AND frc.c_RCIntervalSubscriptionEnd <= @endDate))
	--NRC
		OR (c_NRCIntervalSubscriptionStart >= %%START_DATE%% AND c_NRCIntervalSubscriptionStart < @endDate)
	-- Account Credit
		OR (acr.c_CreditTime >= %%START_DATE%% AND acr.c_CreditTime < @endDate)
	-- Non-Standart Charge
		OR (nsc.c_IssueTime >= %%START_DATE%% AND nsc.c_IssueTime < @endDate)
	-- Account Usage
		OR (udrc.id_usage_interval IS NULL AND frc.id_usage_interval IS NULL AND nrc.id_usage_interval IS NULL AND acc.dt_crt >= %%START_DATE%% AND acc.dt_crt < @endDate)
	)
