...
$(function() {
    ...
    $('#sitemap dl, #sitemap .dropzone').droppable({
        ...
        over: function() {
          $(this).filter('dl').css({ backgroundColor: '#E6E6E6' });
            $(this).filter('.dropzone').css({ borderColor: '#EBB35C' });
        },
        out: function() {
	...
	$('.sm2_expander').live('click', function() {
	  $(this).parent().parent().toggleClass('sm2_liOpen', 150).toggleClass('sm2_liClosed', 150);
		return false;
	});
});
