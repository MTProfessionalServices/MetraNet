...
PUBLIC FUNCTION CustomCheckAccountTypeBusinessRules(objAccountType, objMenuTab)
  ...
  Select Case UCase(objAccountType.Name)
    ...
    ' GSMServiceAccount Menu Rules
    Case "GSMSERVICEACCOUNT"
      
      If Session("SubscriberYAAC").CorporateAccountID = -1  then
        objMenuTab.MenuLinks("Disconnect").Visible = FALSE
      End IF
      
      If MAM().Subscriber("PayerID") = -1  then
        objMenuTab.MenuLinks("subscriptions").Visible = FALSE
      End IF      
