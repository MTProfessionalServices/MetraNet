...
Function CheckDefaultPolicy()
  ...
  If FrameWork.CheckCoarseCapability("Manage Subscriber Auth") Then
  
    'Do nothing if adding corporate account
    if CLng(Form("FolderID")) = MAM_HIERARCHY_ROOT_ACCOUNT_ID then
...
PRIVATE FUNCTION Form_Refresh(EventArg) ' As Boolean
    ...
    ' Set corp. account settings
    If CBool(mam_GetAccountType(Service("AccountType").Value).IsCorporate) Then
      Service.Properties("BILLABLE").Value = TRUE 
     ' Service.Properties("BILLABLE").Enabled = FALSE
      Service.Properties("FOLDER").Value = TRUE 
     ' Service.Properties("FOLDER").Enabled = FALSE
     ' Service.Properties("PayerAccount").Enabled = FALSE
    End If
...
PRIVATE FUNCTION OK_Click(EventArg) ' As Boolean
    ...
    ' Set corp. account settings that must be metered
    If CBool(mam_GetAccountType(Service("AccountType").Value).IsCorporate) Then
      Service.Properties("BILLABLE").Value    = TRUE 
      Service.Properties("BILLABLE").Enabled  = TRUE
      Service.Properties("FOLDER").Value      = TRUE 
      Service.Properties("FOLDER").Enabled    = TRUE
    End If
    ...
    If(CheckIfEMailIsRequiredAndBlank(EventArg))Then
        ...
        Call ResetProperties(EventArg) 
        Exit Function
    End If 
    If(Not mam_Account_CheckBillingCycleRule(EventArg))Then
    
        Call ResetProperties(EventArg) 
        Exit Function
    End If 
    If(Not mam_Account_CheckIfEMailIsValid(EventArg)  )Then
    ...
    If(Not ValidatePassWord(EventArg))Then 
    
        Call ResetProperties(EventArg) 
        Exit Function ' If confirmed password is not equal to password we return to the dialog 
    End If 
    
    mam_Account_SetToEmptyAllTheEmptyStringValue

    If Not mam_Account_ProcessFieldsID() Then 
    
        Call ResetProperties(EventArg) 
        Exit Function ' The error message is taken care of.
    End If 
    ...
    If Len(Service.Properties("PayerAccount").value) Then
      If FrameWork.DecodeFieldID(Service.Properties("PayerAccount").value, PayerAccountID) Then
            ...
            If err.number <> 0 then
              ...
              Exit Function
            End if
            'Service.Properties("PayerAccount").value = PayerAccountID
      End If     
    End If
...
' DESCRIPTION 		:
' RETURNS		      :
PRIVATE FUNCTION ResetProperties(EventArg) ' As Boolean

    ResetProperties = FALSE
  
    ' Reset corp. settings on error
    If CBool(mam_GetAccountType(Service("AccountType").Value).IsCorporate) Then
      Service.Properties("BILLABLE").Value = TRUE 
      Service.Properties("BILLABLE").Enabled = FALSE
      Service.Properties("FOLDER").Value = TRUE 
...
PRIVATE FUNCTION InitializeServiceForAddingANewAccount()
    ...
    ' MAM 3.0
    If ((CLng(Form("FolderID")) = MAM_HIERARCHY_ROOT_ACCOUNT_ID) or (Len(Form("FolderID")) = 0) ) Then
      Service.Properties("CURRENCY").Value = mam_GetDictionary("ADD_ACCOUNT_DEFAULT_CURRENCY")
      Service.Properties("CURRENCY").Enabled = True
    ...
    Else
      ...
      Service.Properties("CURRENCY").Value   = MAM().TempAccount("CURRENCY").Value
    
      ' 3.6 check Hierarchy_RestrictedOperations business rule to decide if the currency can change            
      Dim pc
      ...
      If pc.IsBusinessRuleEnabled(PCCONFIGLib.MTPC_BUSINESS_RULE_Hierarchy_RestrictedOperations) Then
        FrameWork.Dictionary.Add "SHOW_CURRENCY", FALSE
      Else
        FrameWork.Dictionary.Add "SHOW_CURRENCY", TRUE
      End If
    End IF  
