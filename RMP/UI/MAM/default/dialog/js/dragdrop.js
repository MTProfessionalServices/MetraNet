...
function clearSelection(){
     ...
     for(i=0;i < m_arrMultiSelectData.length; i++){
       if(eval("getFrameMetraNet().hierarchy.document.all.text" + m_arrMultiSelectData[i]+ ".style") != null){
       
         eval("getFrameMetraNet().hierarchy.document.all.text" + m_arrMultiSelectData[i] + ".style.color = '" + NORMAL_COLOR + "';");
...
function GetHierarchyPath(strName) {
  if(getFrameMetraNet().hierarchy.getCurrentPath().indexOf("(1)") == -1) {
    eval("document.all." + strName + ".value = getFrameMetraNet().hierarchy.getCurrentPath();");
...
function GetID(strName) {
  var obj = window.event.srcElement;
	...
	{
	  // Added for new MetraCare support
	  var pos = obj_id.indexOf("=");
	  if(pos != -1)
	  {
	    obj_id = obj_id.substr(pos+1, obj_id.length);
	  }
	}
...
function GetFieldID(strName) {
	...
	if(getFrameMetraNet().hierarchy)
	{
	  if(getFrameMetraNet().hierarchy.GetFieldIDFromAccountID)
	  {
		  getFrameMetraNet().hierarchy.GetFieldIDFromAccountID(nID);
	  }
	  else
	  {
		  alert('You must load the hierarchy before drag and drop will work.');  
	  }
	...
	else
	{
	  ...
	  if(pos != -1)
	  {
	    nID = nID.substr(pos+1, nID.length);
	    var str = nID;  // todo: get field name
      str = str.replace(/'/g, "\\\'"); 
...
function waitForReady(strName) {
  if(getFrameMetraNet().hierarchy.LastID != null) {
    var str = getFrameMetraNet().hierarchy.LastID;
    str = str.replace(/'/g, "\\\'"); 
    eval("document.all." + strName + ".value = '" + str + "';");
    getFrameMetraNet().hierarchy.LastID = null;
  }
  else {
...
function HandleDrop() {
  ...
  if((bDropable && bFolder) || (bDropable && (obj.dragID == "DropField"))) {
	...
	{
  	// Added for new MetraCare support
	  var pos = strSelected.indexOf("=");
	  if(pos != -1)
	  {
	    strSelected = strSelected.substr(pos+1, strSelected.length);
	  }
	}
