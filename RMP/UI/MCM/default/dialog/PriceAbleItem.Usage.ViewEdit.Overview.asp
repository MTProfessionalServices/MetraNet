...
'	mdm_GetDictionary().Add "CURRENT_PRICE_ABLE_ITEM_KIND",Form("KIND")
  ...
  Service.Properties.Add "pai_usage_ve_overview_name", "String",  1024, FALSE, TRUE
  Service.Properties("pai_usage_ve_overview_name") = SafeForHtml(COMObject.Instance.Name)
...
' RETURNS		      : Return TRUE if ok else FALSE
PUBLIC FUNCTION Form_Paint(EventArg) ' As Boolean
    ...
    strTemplateFolder             = Session("FRAMEWORK_APP_FOLDER") & "\Custom\customview\"    

    'response.write("strTemplateFolder [" & strTemplateFolder & "]")
    'response.write("Session(FRAMEWORK_APP_FOLDER) [" & Session("FRAMEWORK_APP_FOLDER") & "]")


    'strTemplateFileName = strTemplateFolder & "metratech_com_GSM.htm"
     'strTemplateFileName = strTemplateFolder & "metratech_com_GSM.htm"
     
    'stop  'call 512 con 517  feature 521
    if FORM("ID") = "512" then
     strTemplateFileName = strTemplateFolder & "AudioConfCallImage.htm"
    else
     if FORM("ID") = "521" then
       strTemplateFileName = strTemplateFolder & "AudioConfFeatureImage.htm"
     else
      strTemplateFileName = strTemplateFolder & "AudioConfConnImage.htm"
     end if
    end if
    
    '<iframe src="/mcm/Custom/customview/Sample%20Rate%20Overview%20Diagram_files/jpg_1.htm" width="100%" height="100%"></iframe>

    on error goto 0

	  If(Len(strTemplateFileName))Then
	      Set objTextFile = mdm_CreateObject(CTextFile)
    'response.write("Session(FRAMEWORK_APP_FOLDER) [" & strTemplateFileName & "]")
    ...
    'response.end
        ...
        varBillingCycleHTMLSubDialog = objTextFile.LoadFile(strTemplateFileName)
  	End If
    'varBillingCycleHTMLSubDialog = "Rudi was here"
    EventArg.HTMLRendered = Replace(Cstr(EventArg.HTMLRendered ),"[CUSTOM_OVERVIEW_TEMPLATE] ",CStr(varBillingCycleHTMLSubDialog))
    EventArg.HTMLRendered = Replace(Cstr(EventArg.HTMLRendered ),"[URL]","\mcm\custom\customview")
	  Form_Paint = TRUE
END FUNCTION
%>
