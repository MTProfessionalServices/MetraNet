...
PRIVATE FUNCTION Form_Initialize(EventArg) ' As Boolean
    ...
    Form("ProductOfferingId") = request("ProductOfferingId")
...
PRIVATE FUNCTION Form_LoadProductView(EventArg) ' As Boolean
  ...
  dim rowset
    ...
    if len(request("ProductOfferingId"))>0 then
      Form("ProductOfferingId") = request("ProductOfferingId")
    end if

    set rowset = server.CreateObject("MTSQLRowset.MTSQLRowset.1")
  	rowset.Init "queries\productcatalog"
    rowset.SetQueryTag("__GET_DEPENDENT_SUBSCRIBERS_FOR_PRODUCT_OFFERING__")  
    rowset.AddParam "%%ID_PO%%", CLng(Form("ProductOfferingId"))
  	rowset.Execute
