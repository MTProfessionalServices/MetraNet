...
FUNCTION form_Refresh(EventArg)
  ...
  if rowset.value("Failed")>0 then
    ...
    Service.Properties("FAILEDTRANSACTIONS_HTML_LINK") = "<A href=""#"" onclick=""window.open('/MetraNet/MetraControl/FailedTransactions/FailedTransactionsView.aspx?Filter_FailedTransactionList=N&Filter_FailedTransactionList_BatchId=" & Server.UrlEncode(Service.Properties("BatchId").Value) & "&PageTitle=" & Server.UrlEncode(mdm_GetDictionary().Item("TEXT_FAILED_TRANSACTIONS_FOR_BATCH") & " " & Service.Properties("BatchId").Value) & "','', 'height=600,width=800, resizable=yes, scrollbars=yes, status=yes')"">" & "View Failed Transactions" & "</A>"
