...
Function RenderMenu()
  ...
  dim sUrlForQueryRendering
  sUrlForQueryRendering = "/mom/default/dialog/IntervalManagement.Statistics.Render.asp?LanguageId=840&IntervalId=" & request("IntervalId") & "&"
  
  dim BillingGroupId
  dim sQueryMenuCategory  'Either EOP\Interval  or EOP\BillingGroup
  if (len(request("BillingGroupId"))>0) then
    BillingGroupId = request("BillingGroupId")
    sQueryMenuCategory = "EOP\BillingGroup"
    sUrlForQueryRendering = sUrlForQueryRendering & "BillingGroupId=" & BillingGroupId & "&"
  else
    BillingGroupId = -1
    sQueryMenuCategory = "EOP\Interval"
  end if
  
  'Add the top level
  strHTML = strHTML & "   <table cellspacing=""0"" cellpadding=""0"">" & vbNewline
  strHTML = strHTML & "     <tr valign=""middle"">" & vbNewline
  strHTML = strHTML & "       <td class=""clsMenuRootText"">" & vbNewline
  ...
  if BillingGroupId = -1 then
	strHTML = strHTML & "       <td class=""clsMenuRootText"">Available Interval Statistics" & "</td>" & vbNewline
  else
  	strHTML = strHTML & "       <td class=""clsMenuRootText"">Available Billing Group Statistics" & "</td>" & vbNewline
  end if
  ...
  strMenuXML=objStatisticsQueryConfig.GetMenuXML(sQueryMenuCategory, sUrlForQueryRendering, "fmeStatView")
...
Function OpenMenuGroup(strPos, strName, strText, strStyle, bEmpty)
  ...
  strHTML = strHTML & "   <table cellspacing=""0"" cellpadding=""0"">" & vbNewline
  strHTML = strHTML & "     <tr valign=""middle"">" & vbNewline
  strHTML = strHTML & "       <td class=""" & strStyle & """ width=""1%"">" & vbNewline
  ...
  if bEmpty then
    ...
    strHTML = strHTML & "       <td id=""cell" & strName & """ class=""" & strStyle & """ onClick=""ToggleRow('row" & strName & "', 'img" & strName & "');"">" & vbNewline
  end if
