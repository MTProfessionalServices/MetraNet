 <% 
' ---------------------------------------------------------------------------------------------------------------------------------------
'  Copyright 1998-20001 by MetraTech Corporation
'  All rights reserved.
' 
...
PRIVATE FUNCTION GetXMLMenuFileName() ' As String
    GetXMLMenuFileName  = mom_GetMenuName()
END FUNCTION
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
Function RenderMenu()
  ...
  'Add the top level
  strHTML = strHTML & "   <table cellspacing=""0"" cellpadding=""0"" border=""0"">" & vbNewline
  ...
  strHTML = strHTML & "       <td class=""clsMenuRootText""><a class='clsMenuLink' target='fmeMain' href='Welcome.asp'>" & request.ServerVariables("SERVER_NAME") & "</a></td>" & vbNewline
  strHTML = strHTML & "     </tr>" & vbNewline
  strHTML = strHTML & "   </table>" & vbNewline
  
  'Create the menu
  Set objMenuGroup  = ObjMenuCreator.GetMenu(CStr(GetXMLMenuFileName()), "MetraControl", "", mid(session("VIRTUAL_DIR"), 2), server.MapPath(session("VIRTUAL_DIR")))  
...
Function RenderMenuItem(objMenuItem, strPos)
  ...
  strHTML = strHTML & "       </td>" & vbNewline
  strHTML = strHTML & "       <td class=""clsMenuItemText"">" & vbNewline
  strHTML = strHTML & "         <a class=""clsMenuLink"" target=""" & objMenuItem.target & """ href=""" & objMenuItem.link & """>" 
  
  if len(objMenuItem.icon) > 0 then
    strHTML = strHTML & "         <img border=0 src=""" & mom_GetDictionary("DEFAULT_LOCALIZED_PATH") & "/images/menu/" & objMenuItem.icon & """>" & vbNewline
  else
    strHTML = strHTML & "         <img border=0 src=""" & mom_GetDictionary("DEFAULT_LOCALIZED_PATH") & "/images/menu/menu_link_default.gif"">" & vbNewline
  end if
...
<html>
  <head>
    <title>MetraTech Platform Manager</title>
