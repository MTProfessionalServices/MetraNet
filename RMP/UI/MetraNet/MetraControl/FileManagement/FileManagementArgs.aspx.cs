...
public partial class FileManagementArgs : MTPage
{
  ...
  {
    get { return Request.RawUrl as string; }
    set {  }
  }
  ...
  protected void Page_Load(object sender, EventArgs e)
  {
    ...
    if (!IsPostBack) 
    {
      ...
      // If the requester's url is in the breadcrumb list, then
      // we can use the breadcrumb list to determine the return url.
      // This overrides any previous setting.
      int i = 0;
      string urlReferrer = Request.UrlReferrer.ToString();
      foreach (var buildCrumb in BreadCrumbs)
      {
        if (urlReferrer.Contains(buildCrumb.Url) && i >= 2)
        {
          ReturnUrl = BreadCrumbs[i - 2].Url;
        }
        i++;
      }
