...
public partial class Relationships_AjaxServices_GetAccountBillManagers : MTListServicePage
{

    private const int MAX_RECORDS_PER_BATCH = 50;

    protected bool ExtractDataInternal(RelationshipServiceClient client, ref MTList<AccountBillManager> items, int batchID, int limit)
    {
      try
      {
        items.Items.Clear();
        ...
        client.GetAccountBillManagers(ref items);
      }
    ...
    protected bool ExtractData(RelationshipServiceClient client, ref MTList<AccountBillManager> items)
    {
    ...
    {
      using (new HighResolutionTimer("GetAccountBillManagers", 5000))
      {
        ...
        try
        {
          ...
          var items = new MTList<AccountBillManager>();

          items.Filters.Add(new MTFilterElement("AccountID", MTFilterElement.OperationType.Equal, UI.Subscriber.SelectedAccount._AccountID));
          SetPaging(items);
          SetSorting(items);
          SetFilters(items);
