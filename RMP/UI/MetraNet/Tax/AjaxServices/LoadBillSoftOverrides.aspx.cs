...
public partial class Tax_AjaxServices_LoadBillSoftOverrides : MTListServicePage
{
  private const int MAX_RECORDS_PER_BATCH = 50;

  protected bool ExtractDataInternal(TaxServiceClient client, ref MTList<BillSoftOverride> items, int batchID, int limit)
  {
    try
    {
      items.Items.Clear();
      ...
      client.GetBillSoftOverrides(ref items);
    }
  ...
  protected bool ExtractData(TaxServiceClient client, ref MTList<BillSoftOverride> items)
  {
  ...
  {
    using (new HighResolutionTimer("GetBillSoftOverride", 5000))
    {
      ...
      try
      {
        ...
        var items = new MTList<BillSoftOverride>();
        SetPaging(items);
        SetSorting(items);
        SetFilters(items);
