...
/// </summary>
public partial class UserControls_MetraControlMenu : System.Web.UI.UserControl
{
  protected void Page_Load(object sender, EventArgs e)
  {
    if (Session[Constants.METRACONTROL_MENU] == null)
    {
      MenuHtml.Text = RenderMainMenu();
      Session[Constants.METRACONTROL_MENU] = MenuHtml.Text;
    }
    else
    {
      MenuHtml.Text = Session[Constants.METRACONTROL_MENU] as string;
    }
  }
  ...
  public string RenderMainMenu()
  {
    ...
    string menuFilename = Server.MapPath("/MetraNet") + @"\Config\Menus\MetraControlMenu.xml";
    ...
    try
    {
      menu = mm[Constants.METRACONTROL_MENU];
      if (menu == null)
      {
        throw new Exception(String.Format("{0} not found in {1}", Constants.METRACONTROL_MENU, menuFilename));
      }
    }
    catch (Exception exp)
    {
      ((MTPage)Page).Logger.LogError("Invalid menu file; check to make sure MetraControlMenu.xml is valid: " + exp);
      return String.Format("{0} [{1}]", Resources.ErrorMessages.ERROR_MAIN_MENU, Constants.METRACONTROL_MENU);
    }
