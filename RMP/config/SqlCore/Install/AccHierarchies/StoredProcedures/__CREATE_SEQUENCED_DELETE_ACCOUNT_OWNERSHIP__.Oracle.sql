    ...
    SELECT id_owner, id_owned, id_relation_type, n_percent, dbo.addsecond(p_vt_end) AS vt_start, vt_end, p_tt_current as tt_start, p_tt_max as tt_end
      ...
      INSERT INTO t_acc_ownership(id_owner, id_owned, id_relation_type, n_percent,  vt_start, vt_end, tt_start, tt_end) 
      SELECT id_owner, id_owned, id_relation_type, n_percent, vt_start, dbo.subtractsecond(p_vt_start) AS vt_end, p_tt_current AS tt_start, p_tt_max AS tt_end 
      FROM t_acc_ownership 
	    WHERE id_owner = p_id_owner AND id_owned = p_id_owned
	    AND vt_start < p_vt_start AND vt_end >= p_vt_start AND tt_end = p_tt_max;
      
      UPDATE t_acc_ownership SET tt_end = dbo.subtractsecond(p_tt_current)
	    WHERE id_owner = p_id_owner AND id_owned = p_id_owned
	    AND vt_start < p_vt_start AND vt_end >= p_vt_start AND tt_end = p_tt_max;
