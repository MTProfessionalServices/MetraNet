...
create or replace 
procedure AcknowledgeCheckpoint(
  p_dt_now date,
  p_id_instance int,
...
begin
  ...
  if (p_b_ignore_deps = 'N') then
    ...
    unsatisfied_deps := dbo.GetEventExecutionDeps(p_dt_now,  p_id_instance);
  ...
  where rowid in (
    ...
    where /* event is active */
     ...
     and /* checkpoint must presently be in the NotYetRun state */
          inst.tx_status in ('NotYetRun')
  ...
  if (sql%rowcount = 1) then
    ...
    values (seq_t_recevent_inst_audit.nextval, p_id_instance, p_id_acc,
        'Acknowledge', p_b_ignore_deps, null, p_tx_detail, p_dt_now);
