    ...
    begin
          Declare
              ...
              cnt NUMBER;
          begin
              l_system_datetime := %%%SYSTEMDATE%%%;
              l_system_date := dbo.MTStartOfDay(l_system_datetime);
              l_max_datetime := dbo.MTMaxDate();
              ...
              t_account_mapper map where upper(tmp.nm_login) = upper(map.nm_login)
              AND upper(tmp.nm_space) = upper(map.nm_space));
              ...
              UPDATE %%TMP_TABLE_NAME%% tmp
							...
							WHERE tmp.status IS NULL
								AND lower(tmp.nm_login) NOT IN ('rm', 'mps_folder')
								AND tmp.acc_type <> 'SYSTEMACCOUNT'
								and exists (
												select 1 
												from t_namespace nmsp 
												where tmp.nm_space = nmsp.nm_space
													and lower(nmsp.tx_typ_space) IN ('system_auth', 'system_mcm',
															'system_ops',  'system_rate', 'system_user')
														);
              ...
              SET    tmp.status = -486604732  /* MT_ACCOUNT_TYPE_AND_NAMESPACE_MISMATCH (0xE2FF0046)*/
              WHERE tmp.status IS NULL
                AND upper(tmp.nm_login) NOT IN ('RM', 'MPS_FOLDER')
                AND upper(tmp.acc_type) = 'SYSTEMACCOUNT'
                AND EXISTS ( 
									select 1 from 
                  t_namespace nmsp where UPPER(tmp.nm_space) = UPPER(nmsp.nm_space)
              ...
              WHERE tmp.status IS NULL
                 ...
                 and exists (
											select 1 from t_account_type atype 
											where upper(atype.name) = upper(tmp.acc_type)
              ...
              SET    tmp.acc_vtstart = CASE WHEN tmp.acc_vtstart IS NULL THEN l_system_date
                    ...
                    tmp.acc_vtend = CASE WHEN tmp.acc_vtend IS NULL THEN l_max_datetime
                                                                    ...
                                                                    ELSE trunc(tmp.acc_vtend) + 1 - 1/(24*60*60) END,
              ...
              SELECT id_account, acc_state, acc_vtstart, acc_vtend, l_system_datetime, l_max_datetime
              FROM %%TMP_TABLE_NAME%% tmp
              WHERE tmp.status IS NULL;
              ...
              OPEN l_CursorVar for 'SELECT DISTINCT nm_space, langcode FROM %%TMP_TABLE_NAME%% WHERE status IS NULL';
              ...
              for i in (select tmp.rowid tmprowid, site.id_site id_site from 
                        %%TMP_TABLE_NAME%% tmp,t_localized_site site 
                        where tmp.status IS NULL AND 
                        upper(tmp.nm_space) = upper(site.nm_space)
                        AND tmp.langcode = site.tx_lang_code) 
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.id_site = i.id_site
                  WHERE  rowid = i.tmprowid;
              END LOOP;
			  ...
			  select count(*) into cnt from %%TMP_TABLE_NAME%% where id_cycle_type is not null;
			  if (cnt > 0) then
      	             
              for i in ( select tmp.rowid rowid1, tuc.id_usage_cycle id_usage_cycle 
                        from %%TMP_TABLE_NAME%% tmp, t_usage_cycle tuc WHERE
                        tmp.status IS NULL AND
              ...
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.id_usage_cycle = i.id_usage_cycle
            ...
            be associated later via 'usm -create'.
                ...
                begin
                  select l_system_date + nvl(n_adv_interval_creation,0) into l_dt_end FROM t_usage_server;
                  exception 
                  when NO_DATA_FOUND THEN
                    l_dt_end := l_system_date;
                end;
		  ...
		  NULL
              ...
              SET    tmp.hierarchy_start = CASE WHEN tmp.hierarchy_start IS NULL THEN tmp.acc_vtstart
                    ...
                    tmp.id_ancestor_out = CASE WHEN tmp.ancestor_name IS NOT NULL
                                                ...
                                                THEN
                                                    ...
                                                    WHERE upper(acc.nm_login) = upper(tmp.ancestor_name)
                                                      AND upper(acc.nm_space) = upper(tmp.ancestor_namespace))
                                                ELSE
                                                    /* If the ancestor id is NULL default to root.*/
                                                    CASE WHEN (tmp.id_ancestor IS NULL OR tmp.id_ancestor = 1) THEN 1
                                                                                      ELSE (select acc.id_acc from t_account acc
																							where acc.id_acc = tmp.id_ancestor) END
                                                END
              WHERE  tmp.status IS NULL;
              ...
              /* Step 7c: Find the type of the ancestor .*/

              update %%TMP_TABLE_NAME%% tmp
              set tmp.ancestor_type = (select atype.name 
                                        from t_account_type atype
                                        inner join t_account acc
                                        on acc.id_type = atype.id_type
                   						where tmp.status is NULL
										and tmp.id_ancestor_out is not NULL
                                        and tmp.id_ancestor_out = acc.id_acc)
              where exists ( select atype.name 
                                        from t_account_type atype
                                        inner join t_account acc
                                        on acc.id_type = atype.id_type
                   						where tmp.status is NULL
              ...
              WHERE  tmp.status IS NULL
                AND exists
                ( select 1 from t_account_ancestor anc 
                    where tmp.id_account = anc.id_ancestor
			  ...
			  update %%TMP_TABLE_NAME%% tmp
					...
					where tmp.status is NULL
					and tmp.id_request not in (select tmp1.id_request from %%TMP_TABLE_NAME%% tmp1
              ...
              /* Step 7h: Update each parent entry so that they indicate they have children.*/
							UPDATE t_account_ancestor anc
							SET anc.b_children = 'Y'
							where exists
							(select 1 from %%TMP_TABLE_NAME%% tmp where anc.id_descendent = tmp.id_ancestor_out AND anc.vt_start <= tmp.hierarchy_end 
												 AND tmp.hierarchy_start <= anc.vt_end and tmp.status IS NULL)               
               and Anc.id_descendent in (select tmp2.id_ancestor_out FROM %%TMP_TABLE_NAME%% TMP2)
               and anc.b_children <> 'Y';

							/* Populate t_dm_account and t_dm_account_ancestor table */
							insert into t_dm_account select seq_t_dm_account.nextval,anc.id_descendent, anc.vt_start, anc.vt_end from
							t_account_ancestor anc INNER JOIN %%TMP_TABLE_NAME%% tmp ON 
              ...
              SET    tmp.id_payer = CASE WHEN tmp.payer_login IS NOT NULL
                                        ...
                                        THEN
                                              ...
                                              (SELECT acc.id_acc
                                                FROM t_account_mapper acc
                                                WHERE ((upper(acc.nm_login) = upper(tmp.payer_login))
                                                  AND (upper(acc.nm_space) = upper(tmp.payer_namespace))))
              ...
              where tmp.status IS NULL
                AND exists
              (
              select 1 from t_account_state ast where tmp.id_payer = ast.id_acc
                                            /* The next AND clause used to be:
                                               AND (dbo.EnclosedDateRange(ast.vt_start, ast.vt_end, tmp.payer_startdate, tmp.payer_enddate) = 1)*/
                                            AND ((CASE WHEN (tmp.payer_enddate >= ast.vt_start
                                                          AND tmp.payer_startdate <= ast.vt_end)
                                                        THEN 1 ELSE 0 END) = 1)
              ...
              AND EXISTS
              (
              select 1 from t_av_internal int where tmp.id_payer = int.id_acc
              ...
              for i in (select tmp.rowid rowid1, parentcorp.id_ancestor id_ancestor1,desccorp.id_ancestor id_ancestor2  
                        from %%TMP_TABLE_NAME%% tmp, t_account_ancestor descendent,t_account_ancestor parent ,t_account_ancestor parentcorp,t_account_ancestor desccorp 
                                            where tmp.status IS NULL
                                            AND tmp.id_payer <> -1
                                            AND descendent.id_descendent = tmp.id_payer
                                            AND tmp.payer_startdate BETWEEN descendent.vt_start AND descendent.vt_end
                                            AND descendent.id_ancestor = 1
                                            AND parent.id_descendent = tmp.id_account
                                            AND parent.id_ancestor = 1
                                            AND tmp.payer_startdate BETWEEN parent.vt_start AND parent.vt_end
                                            AND parentcorp.id_descendent = tmp.id_account
                                            ...
                                            AND desccorp.id_descendent = tmp.id_payer
              ...
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.same_corporation = CASE WHEN i.id_ancestor1 = i.id_ancestor2
                                                    THEN 1
                                                    ELSE 0 END
                  where rowid = i.rowid1;
              END LOOP;
              ...
              UPDATE %%TMP_TABLE_NAME%%
              SET    auth_ancestor = id_ancestor_out
              WHERE  status IS NULL
                AND apply_default_policy = 'T'
                AND UPPER(acc_type) <>  'SYSTEMACCOUNT';
              ...
              SET  parent_login = CASE WHEN login_app = 'CSR' THEN 'csr_folder'
                                  WHEN login_app = 'MCM' THEN 'mcm_folder'
                                  WHEN login_app = 'MOM' THEN 'mom_folder'
                                  WHEN login_app = 'MPS' THEN 'mps_folder'
                                  ELSE NULL END
              WHERE  status IS NULL
			        AND apply_default_policy = 'T'
			        AND UPPER(acc_type) = 'SYSTEMACCOUNT';
              ...
              --          policy should be inherited from for non-subscriptions.
              for i in                 (
                select tmp.rowid rowid1, map.id_acc id_acc from %%TMP_TABLE_NAME%% tmp,t_account_mapper map
                where tmp.status IS NULL
                AND upper(tmp.parent_login) = upper(map.nm_login)
                AND upper(map.nm_space) = 'AUTH')
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.auth_ancestor = i.id_acc
                  WHERE  rowid= i.rowid1;
              END LOOP;
              ...
              for i in ( select tmp.rowid rowid1,pr.id_policy id_policy from %%TMP_TABLE_NAME%% tmp, t_principal_policy pr where pr.id_acc = tmp.auth_ancestor
                                              AND pr.policy_type= 'D' AND tmp.status IS NULL)
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.parent_policy = i.id_policy
                  WHERE  ROWID = i.rowid1;
              END LOOP;
              ...
              for i in (
                  select tmp.rowid rowid1,pr.id_policy id_policy from %%TMP_TABLE_NAME%% tmp,t_principal_policy pr 
                  where tmp.status IS NULL AND pr.id_acc = tmp.id_account
                                              AND pr.policy_type= 'A')
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.child_policy = i.id_policy
                  WHERE  rowid = i.rowid1;
              END LOOP;
              ...
              for i in (
               select tmp.rowid rowid1, pr.id_policy id_policy from %%TMP_TABLE_NAME%% tmp,t_principal_policy pr where tmp.status IS NULL
                AND tmp.child_policy IS NULL AND pr.id_acc = tmp.id_account
                                            AND pr.policy_type = 'A')
              LOOP
                  UPDATE %%TMP_TABLE_NAME%% tmp
                  SET    tmp.child_policy = i.id_policy
              ...
              for i in (
              select tmp.rowid rowid1,anc.tx_path tx_path from %%TMP_TABLE_NAME%%  tmp, t_account_ancestor anc where  tmp.status IS NULL 
                                                AND tmp.id_account = anc.id_descendent
                                                AND (anc.id_ancestor = 1 OR anc.id_ancestor = -1)
                                                AND tmp.hierarchy_start BETWEEN anc.vt_start AND anc.vt_end)
              LOOP
                  UPDATE %%TMP_TABLE_NAME%%  tmp
                  SET    tmp.hierarchy_path = i.tx_path
                  WHERE  rowid=i.rowid1;
              END LOOP;
              ...
              WHERE  tmp.status IS NULL
                ANd Exists
                (select 1 from t_av_internal int where tmp.id_ancestor_out = int.id_acc
                AND %%ENFORCE_SAME_CORPORATION%% = 1
                AND int.c_currency <> tmp.account_currency);
          end;
    end;
