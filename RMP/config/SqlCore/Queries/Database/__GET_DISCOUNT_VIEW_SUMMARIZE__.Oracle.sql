				...
				au.am_currency Currency, count(1) Count, sum(nvl(pv.c_DiscountTaxAmount, 0.0)) TaxAmount,
				sum(nvl(pv.c_DiscountAmount, 0.0) + nvl(pv.c_DiscountTaxAmount, 0.0)) AmountWithTax, %%ACCOUNT_ID%% AccountID, %%INTERVAL_ID%% IntervalID from
