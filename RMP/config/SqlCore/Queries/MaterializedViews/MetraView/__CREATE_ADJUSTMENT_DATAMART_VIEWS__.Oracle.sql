	   ...
	   begin
			execute immediate 'CREATE or replace force view VW_ADJUSTMENT_SUMMARY_DATAMART as
				select
       ...
       ,SUM (CASE WHEN ajtrx.n_adjustmenttype=0  THEN 1 ELSE 0 END)  AS NumPrebillAdjustments
				...
				and nvl(ajtrx.id_sess,ajtrx.archive_sess) <=(select id_sess from t_mv_max_sess)
				GROUP BY
				ajtrx.id_acc_payer,
				ajtrx.id_usage_interval,
				ajtrx.am_currency,
			...
			execute immediate 'CREATE view VW_ADJUSTMENT_DETAILS_DATAMART
				as select distinct
				ajt.id_adj_trx,
				ajt.id_reason_code,
				ajt.id_acc_creator,
				...
				CASE WHEN (rcbp.nm_name IS NULL) THEN N'''' ELSE rcbp.nm_name END  AS ReasonCodeName,
				CASE WHEN (rcbp.nm_desc IS NULL) THEN N'''' ELSE rcbp.nm_desc END  AS ReasonCodeDescription,
				CASE WHEN (rcdesc.tx_desc IS NULL) THEN N'''' ELSE rcdesc.tx_desc END  AS ReasonCodeDisplayName,
