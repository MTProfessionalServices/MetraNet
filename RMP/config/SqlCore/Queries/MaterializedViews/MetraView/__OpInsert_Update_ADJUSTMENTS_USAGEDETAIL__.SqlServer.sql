...
--Propogate the changes to the summary delta tables      
declare @rowcount5 int

Truncate table %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_adj_usagedetail

Insert into %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_adj_usagedetail
			...
			au.am_currency,
			au.id_acc,
			isnull(prebillajs.c_status,postbillajs.c_status) c_status,
			...
			from
			t_acc_usage au 
			left outer join %%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%% prebillajs on prebillajs.id_sess=au.id_sess 
			AND prebillajs.c_status IN ('A', 'P') AND prebillajs.n_adjustmenttype = 0
			left outer join %%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%% postbillajs on postbillajs.id_sess=au.id_sess 
			...
			from
				%%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%% childprebillajs 
			...
			from
				%%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%% childpostbillajs
      ...
      LEFT OUTER JOIN t_acc_usage_interval ajaui on postbillajs.id_usage_interval = ajaui.id_usage_interval AND postbillajs.id_acc_payer = ajaui.id_acc
			...
			where  
			exists ( select 1 from %%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%%
						where %%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%%.id_sess = au.id_sess
						and %%DELTA_INSERT_T_ADJUSTMENT_TRANSACTION%%.n_adjustmenttype in (0,1)
						and c_status in ('A','P')
					)
group by
			au.id_sess,
			...
			au.am_currency,
			au.id_acc,
			isnull(prebillajs.c_status,postbillajs.c_status)
...
inner join  %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_adj_usagedetail tmp2 on
...
begin
update dm_1 set
      dm_1.CompoundPostbillFedTaxAdjAmt = IsNULL(tmp2.CompoundPostbillFedTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillStateTaxAdjAmt =  IsNULL(tmp2.CompoundPostbillStateTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillCntyTaxAdjAmt = IsNULL(tmp2.CompoundPostbillCntyTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillLocalTaxAdjAmt =  IsNULL(tmp2.CompoundPostbillLocalTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillOtherTaxAdjAmt =  IsNULL(tmp2.CompoundPostbillOtherTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillTotalTaxAdjAmt =  IsNULL(tmp2.CompoundPostbillTotalTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillFedTaxAdjAmt =  IsNULL(tmp2.AtomicPostbillFedTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillStateTaxAdjAmt =    IsNULL(tmp2.AtomicPostbillStateTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillCntyTaxAdjAmt =   IsNULL(tmp2.AtomicPostbillCntyTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillLocalTaxAdjAmt =    IsNULL(tmp2.AtomicPostbillLocalTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillOtherTaxAdjAmt =    IsNULL(tmp2.AtomicPostbillOtherTaxAdjAmt, 0.0), 
      dm_1.AtomicPostbillTotalTaxAdjAmt =    IsNULL(tmp2.AtomicPostbillTotalTaxAdjAmt, 0.0), 
      dm_1.CompoundPostbillAdjAmt =          IsNULL(tmp2.CompoundPostbillAdjAmt, 0.0), 
      dm_1.CompoundPostbillAdjedAmt =            IsNULL(tmp2.CompoundPostbillAdjedAmt, 0.0) + isnull(dm_1.CompoundPrebillAdjAmt,0.0), 
      dm_1.AtomicPostbillAdjAmt =            IsNULL(tmp2.AtomicPostbillAdjAmt, 0.0), 
      dm_1.AtomicPostbillAdjedAmt =              IsNULL(tmp2.AtomicPostbillAdjedAmt, 0.0) + isnull(dm_1.AtomicPrebillAdjAmt,0.0), 
      dm_1.PostbillAdjustmentID = (CASE WHEN tmp2.PostbillAdjustmentID != -1 THEN tmp2.PostbillAdjustmentID else dm_1.PostbillAdjustmentID END),
      dm_1.IsAdjusted = tmp2.IsAdjusted,
      dm_1.IsPostbillAdjusted = (CASE WHEN tmp2.IsPostbillAdjusted ='Y' or dm_1.IsPostbillAdjusted ='Y' THEN 'Y' else 'N' END),
      dm_1.IsPrebillTransaction = (CASE WHEN tmp2.IsPrebillTransaction ='Y' or dm_1.IsPrebillTransaction ='Y' THEN 'Y' else 'N' END),
      dm_1.CanAdjust = tmp2.CanAdjust,
      dm_1.CanRebill = tmp2.CanRebill,
      dm_1.CanManageAdjustments = tmp2.CanManageAdjustments,
      dm_1.IsIntervalSoftClosed = tmp2.IsIntervalSoftClosed,
      dm_1.NumTransactions = isnull(dm_1.NumTransactions,0) + isnull(tmp2.NumTransactions,0)
	from %%ADJUSTMENTS_USAGEDETAIL%% dm_1 
	inner join  %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_adj_usagedetail tmp2 on
					dm_1.id_sess=tmp2.id_sess 
      and dm_1.id_usage_interval=tmp2.id_usage_interval 
...
Select 
			...
			AtomicPostbillCntyTaxAdjAmt,AtomicPostbillLocalTaxAdjAmt,
			AtomicPostbillOtherTaxAdjAmt,AtomicPostbillTotalTaxAdjAmt,
			CompoundPostbillAdjAmt,CompoundPostbillAdjedAmt,
			AtomicPostbillAdjAmt,AtomicPostbillAdjedAmt,PrebillAdjustmentID,
			PostbillAdjustmentID,IsAdjusted,IsPrebillAdjusted,IsPostbillAdjusted,
			IsPrebillTransaction,CanAdjust,CanRebill,CanManageAdjustments,IsIntervalSoftClosed,NumTransactions
from  %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_adj_usagedetail tmp2 
where not exists (select 1 from %%ADJUSTMENTS_USAGEDETAIL%% dm_1 where 
					dm_1.id_sess=tmp2.id_sess 
      and dm_1.id_usage_interval=tmp2.id_usage_interval 
      and dm_1.am_currency=tmp2.am_currency 
      and dm_1.id_acc=tmp2.id_acc
