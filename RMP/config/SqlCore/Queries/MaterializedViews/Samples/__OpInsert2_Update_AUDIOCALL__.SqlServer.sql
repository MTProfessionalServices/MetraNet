...
inner join %%DELTA_INSERT_T_PV_AUDIOCONFCALL%% call with(readcommitted) 
...
from %%DELTA_INSERT_T_ACC_USAGE%% au1 with(readcommitted) 
left outer join T_PV_AUDIOCONFCONNECTION child1 with(readcommitted) 
on au1.id_sess=child1.id_sess and au1.id_usage_interval=child1.id_usage_interval
left outer join T_PV_AUDIOCONFFEATURE child2 with(readcommitted) 
