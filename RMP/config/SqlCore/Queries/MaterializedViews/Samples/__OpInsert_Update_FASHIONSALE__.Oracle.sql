...
delete from %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale;

Insert into %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale
select 
...
inner join %%DELTA_INSERT_T_PV_FASHIONSALE%% pv  
...
/* Update the existing rows in the MV table and keep all the changes in the delta_delete and delta_insert mv table */
 Insert into %%DELTA_DELETE_FASHIONSALE%% select dm_1.*
      from %%FASHIONSALE%% dm_1 inner join %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale tmp2 
...
update %%FASHIONSALE%% dm_1 set 
	  (dm_1.taxes,dm_1.c_totalprice,dm_1.NumTransactions) = (select nvl(dm_1.taxes,0.0) + nvl(tmp2.taxes, 0.0),
       ...
       nvl(dm_1.NumTransactions,0.0) + nvl(tmp2.NumTransactions, 0.0) from %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale tmp2 
      where dm_1.id_acc=tmp2.id_acc
      and dm_1.id_usage_interval=tmp2.id_usage_interval)
     where exists (select 1 from  %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale tmp2 
      where dm_1.id_acc=tmp2.id_acc
      and dm_1.id_usage_interval=tmp2.id_usage_interval);
...
insert into %%DELTA_INSERT_FASHIONSALE%% select tmp2.* from %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale tmp2 
where not exists 
...
from %%%NETMETERSTAGE_PREFIX%%%summ_delta_i_fashionsale tmp2 where not exists 
