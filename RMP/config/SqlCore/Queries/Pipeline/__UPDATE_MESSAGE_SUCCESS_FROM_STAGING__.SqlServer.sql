...
FROM
t_session_state WITH(READCOMMITTED)
WHERE 
EXISTS (
  SELECT 1 FROM %%%NETMETERSTAGE_PREFIX%%%t_resubmit_transaction_stage stg WITH(READCOMMITTED)
...
FROM 
%%%NETMETERSTAGE_PREFIX%%%t_resubmit_transaction_stage stg WITH(READCOMMITTED) 
