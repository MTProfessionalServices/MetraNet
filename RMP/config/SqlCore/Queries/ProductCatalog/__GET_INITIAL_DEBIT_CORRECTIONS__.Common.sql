...
/*  __GET_INITIAL_DEBIT_CORRECTIONS__ */
SELECT
tmp.c_RCActionType,
tmp.c__AccountID,
...
/*  End Initial Debit Correction Rule */
...
/*  Debit Corrections Rule for initial charges:  */
/*  1) Process subscriptions that start in the current interval */
/*  2) Correct recurring charge interval that begins prior to end of current interval */
/*  3) Charge was billed using payment history version that was (transaction time) */
/*  effective at the end of the interval in which it was billed. */
  AND 
  tmp1.c_SubscriptionStart BETWEEN prevbinterval.dt_start AND prevbinterval.dt_end
  AND
...
INNER JOIN t_recevent_inst rei ON rei.id_event=tmp1.run_id_event AND rei.id_arg_interval=prevbinterval.id_interval AND rei.id_arg_billgroup=bg.id_billgroup
INNER JOIN (
  select
  rer.id_instance,
