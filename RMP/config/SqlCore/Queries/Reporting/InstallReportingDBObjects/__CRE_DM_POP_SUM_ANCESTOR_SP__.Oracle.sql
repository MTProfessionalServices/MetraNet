...
CREATE OR REPLACE PROCEDURE MTSP_DM_POP_SUM_ANCESTOR
AS
BEGIN

DECLARE
	v_id_enum_data NUMBER(38);
	v_owner VARCHAR2(30);

BEGIN
EXECUTE IMMEDIATE ('TRUNCATE TABLE DM_T_SUM_GEN1');
COMMIT;
...
EXECUTE IMMEDIATE ('CREATE INDEX idx_dm_t_sum_gen1_descendent ON dm_t_sum_gen1(id_descendent)');
COMMIT;
SELECT owner
INTO v_owner
FROM all_tables
WHERE table_name = 'DM_T_SUM_GEN1'
AND ROWNUM = 1;

DBMS_STATS.GATHER_TABLE_STATS (ownname => v_owner, tabname => 'DM_T_SUM_GEN1', granularity => 'ALL', cascade => TRUE);

/* IF v_maxgen = 5 THEN */
EXECUTE IMMEDIATE ('TRUNCATE TABLE dm_t_sum_acc_gen_flat');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_1');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_2');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_3');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_4');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_5');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_6');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_7');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_8');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_9');
EXECUTE IMMEDIATE ('DROP INDEX idx_dm_sum_accgenflat_10');
COMMIT;
		...
		FROM
			...
			FROM
				(SELECT
					...
					WHEN aa1.vt_start IS NULL AND tmpd.dt_crt IS NULL THEN TO_DATE('1/1/1970','mm/dd/yyyy')
					END vt_start,
					CASE
					WHEN aa1.vt_end IS NOT NULL THEN aa1.vt_end
					ELSE TO_DATE('1/1/2038','mm/dd/yyyy')
					END vt_end
				FROM t_account tmpd
					INNER JOIN t_av_internal avi
...
INTO v_owner
FROM all_tables
WHERE table_name = 'DM_T_SUM_ACC_GEN_FLAT'
AND ROWNUM = 1;

DBMS_STATS.GATHER_TABLE_STATS (ownname => v_owner, tabname => 'DM_T_SUM_ACC_GEN_FLAT', granularity => 'ALL', cascade => TRUE);

EXECUTE IMMEDIATE ('DROP INDEX idx_dm_t_sum_gen1_descendent');

/*  Generation level table */
EXECUTE IMMEDIATE ('TRUNCATE TABLE DM_T_SUM_ACC_LVL_FLAT');
BEGIN
EXECUTE IMMEDIATE ('DROP INDEX IDX_DM_T_SUM_ACC_LVL_FLAT');
...
SELECT /* gen 0  */
	...
	CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
UNION ALL /* gen 1 */
SELECT 
	gen_0, gen_1 TOP_LVL_1, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
...
UNION ALL /* gen 2 */
SELECT 
	gen_0, gen_2 TOP_LVL_1, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_1 TOP_LVL_2, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_0 TOP_LVL_3, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
UNION ALL /* gen 3 */
SELECT 
	gen_0, gen_3 TOP_LVL_1, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_2 TOP_LVL_2, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_1 TOP_LVL_3, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_0 TOP_LVL_4, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
UNION ALL /* gen 4 */
SELECT 
	gen_0, gen_4 TOP_LVL_1, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_3 TOP_LVL_2, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_2 TOP_LVL_3, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_1 TOP_LVL_4, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_0 TOP_LVL_5, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
UNION ALL /* gen 5 */
SELECT 
	gen_0, gen_5 TOP_LVL_1, CASE WHEN GEN_5_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_4 TOP_LVL_2, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_3 TOP_LVL_3, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_2 TOP_LVL_4, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_1 TOP_LVL_5, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_5_DESC, /* level 5 */
	gen_0 TOP_LVL_6, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
/*  gen 6 */
SELECT 
	gen_0, gen_6 TOP_LVL_1, CASE WHEN GEN_6_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_5 TOP_LVL_2, CASE WHEN GEN_5_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_4 TOP_LVL_3, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_3 TOP_LVL_4, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_2 TOP_LVL_5, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_5_DESC, /* level 5 */
	gen_1 TOP_LVL_6, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_6_DESC, /* level 6 */
	gen_0 TOP_LVL_7, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
/*  gen 7 */
SELECT 
	gen_0, gen_7 TOP_LVL_1, CASE WHEN GEN_7_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_6 TOP_LVL_2, CASE WHEN GEN_6_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_5 TOP_LVL_3, CASE WHEN GEN_5_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_4 TOP_LVL_4, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_3 TOP_LVL_5, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_5_DESC, /* level 5 */
	gen_2 TOP_LVL_6, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_6_DESC, /* level 6 */
	gen_1 TOP_LVL_7, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_7_DESC, /* level 7 */
	gen_0 TOP_LVL_8, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
/*  gen 8 */
SELECT 
	gen_0, gen_8 TOP_LVL_1, CASE WHEN GEN_8_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_8_firstname) || ' ' || RTRIM(gen_8_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_8_firstname) || ' ' || RTRIM(gen_8_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_7 TOP_LVL_2, CASE WHEN GEN_7_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_6 TOP_LVL_3, CASE WHEN GEN_6_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_5 TOP_LVL_4, CASE WHEN GEN_5_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_4 TOP_LVL_5, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_5_DESC, /* level 5 */
	gen_3 TOP_LVL_6, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_6_DESC, /* level 6 */
	gen_2 TOP_LVL_7, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_7_DESC, /* level 7 */
	gen_1 TOP_LVL_8, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_8_DESC, /* level 8 */
	gen_0 TOP_LVL_9, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
/*  gen 9 */
SELECT 
	gen_0, gen_9 TOP_LVL_1, CASE WHEN GEN_9_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_9_firstname) || ' ' || RTRIM(gen_9_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_9_firstname) || ' ' || RTRIM(gen_9_lastname)
	END TOP_LVL_1_DESC, /* level 1 */
	gen_8 TOP_LVL_2, CASE WHEN GEN_8_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_8_firstname) || ' ' || RTRIM(gen_8_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_8_firstname) || ' ' || RTRIM(gen_8_lastname)
	END TOP_LVL_2_DESC, /* level 2 */
	gen_7 TOP_LVL_3, CASE WHEN GEN_7_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_7_firstname) || ' ' || RTRIM(gen_7_lastname)
	END TOP_LVL_3_DESC, /* level 3 */
	gen_6 TOP_LVL_4, CASE WHEN GEN_6_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_6_firstname) || ' ' || RTRIM(gen_6_lastname)
	END TOP_LVL_4_DESC, /* level 4 */
	gen_5 TOP_LVL_5, CASE WHEN GEN_5_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_5_firstname) || ' ' || RTRIM(gen_5_lastname)
	END TOP_LVL_5_DESC, /* level 5 */
	gen_4 TOP_LVL_6, CASE WHEN GEN_4_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_4_firstname) || ' ' || RTRIM(gen_4_lastname)
	END TOP_LVL_6_DESC, /* level 6 */
	gen_3 TOP_LVL_7, CASE WHEN GEN_3_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_3_firstname) || ' ' || RTRIM(gen_3_lastname)
	END TOP_LVL_7_DESC, /* level 7 */
	gen_2 TOP_LVL_8, CASE WHEN GEN_2_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_2_firstname) || ' ' || RTRIM(gen_2_lastname)
	END TOP_LVL_8_DESC, /* level 8 */
	gen_1 TOP_LVL_9, CASE WHEN GEN_1_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_1_firstname) || ' ' || RTRIM(gen_1_lastname)
	END TOP_LVL_9_DESC, /* level 9 */
	gen_0 TOP_LVL_10, CASE WHEN GEN_0_FOLDER_FLAG = 'Y' THEN 'FOLDER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
	ELSE 'MEMBER:   ' || RTRIM(gen_0_firstname) || ' ' || RTRIM(gen_0_lastname)
...
INTO v_owner
FROM all_tables
WHERE table_name = 'DM_T_SUM_ACC_LVL_FLAT'
AND ROWNUM = 1;

DBMS_STATS.GATHER_TABLE_STATS (ownname => v_owner, tabname => 'DM_T_SUM_ACC_LVL_FLAT', granularity => 'ALL', cascade => TRUE);

END;
END MTSP_DM_POP_SUM_ANCESTOR;
