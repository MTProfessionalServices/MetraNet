...
'*
'* These functions upgrade MetraTech Platform database from V4.0 to V4.1
'*
'******************************************************************************************
...
Const kDBState_DB_40_PRESENT		= "4.0"
...
Public Function Main() ' As Boolean
	...
	if (DBStatus = kDBState_DB_40_PRESENT) then
		' call the upgrade database function to upgrade from 4.0.1 to 5.0
		UpgradeDatabaseSQLFromV40
	else 
...
Function UpgradeDatabaseSQLFromV40
	...
	writelog("*** Upgrading Core Database from v4.0 ... ***")

	writelog("*** Update Database Schema...***")	
	strInFile = strInstallDir & "RMP\config\upgrade\V401to50\v401to50.sql"
	strOutFile = strInstallDir & "RMP\config\upgrade\V401to50\v401to50.txt"
	...
	strInFile = strInstallDir & "RMP\Config\upgrade\V401to50\Procedure.sql"
	strOutFile = strInstallDir & "RMP\Config\upgrade\V401to50\Procedure.txt"
	strFilename = "osql.exe"
	strArguements = " -l 3000 -S " & strDBHostName & " -d " & strDBName & " -U " & strAdminID & " -P " & """" & strAdminPwd & """" & " -i " & strInFile & " -o " & strOutFile
	intstatus = WshShell.Run(strfilename & strArguements,VBHide, true)
	...
	writelog("setting up service defintions...")
	If Not RunHook("Setting up service defintions", "MetraTech.Product.Hooks.ServiceDefHook", "") Then Exit Function
	
	writelog("setting up Account Types...")
	If Not RunHook ("Account Types and Views", "MetraTech.Product.Hooks.AccountTypeHook", "") Then Exit Function
    
    	writelog("*** Update Database Schema...***")	
	strInFile = strInstallDir & "RMP\config\upgrade\V401to50\post_acctypehook.sql"
	strOutFile = strInstallDir & "RMP\config\upgrade\V401to50\post_acctypehook.txt"
	strFilename = "osql.exe"
	strArguements = " -l 3000 -S " & strDBHostName & " -d " & strDBName & " -U " & strAdminID & " -P " & """" & strAdminPwd & """" & " -i " & strInFile & " -o " & strOutFile
	intstatus = WshShell.Run(strfilename & strArguements,VBHide, true)
	if (intstatus <> 0) then
	...
	writelog("*** Upgraded Core Database from v4.0.1 to 5.0 successfully ... ***")
