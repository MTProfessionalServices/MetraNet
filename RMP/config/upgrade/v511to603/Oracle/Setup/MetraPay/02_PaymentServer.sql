...
BEGIN
    ...
    from user_objects 
    where object_type = 'TABLE' and object_name = 'T_PS_CREDITCARD_OLD';
    IF cnt > 0 THEN
        select count(*) INTO cnt 
        from user_objects 
        where object_type = 'TABLE' and object_name = 'T_PS_CREDITCARD';
        IF cnt > 0 THEN
            EXECUTE IMMEDIATE  'DROP TABLE T_PS_CREDITCARD';
            cnt := 0;
        END IF;
        cnt := 0;
        EXECUTE IMMEDIATE 'ALTER TABLE T_PS_CREDITCARD_OLD RENAME TO T_PS_CREDITCARD';
    END IF;
END;
/
...
(
  id_payment_instrument NVARCHAR2(40) NOT NULL,
  NM_CUSTOMER         NVARCHAR2(40),
  NM_CCNUM            VARCHAR2(32 BYTE)         NOT NULL,
  NM_CCSECCODE        VARCHAR2(16 BYTE),
  NM_PRIMARY          VARCHAR2(1 BYTE)          NOT NULL,
  NM_ENABLED          VARCHAR2(1 BYTE)          NOT NULL,
  NM_AUTHRECEIVED     VARCHAR2(1 BYTE)          NOT NULL,
  ID_ACC              NUMBER(10)                NOT NULL,
  NM_ADDRESS          NVARCHAR2(255),
  NM_CITY             NVARCHAR2(20),
  NM_STATE            NVARCHAR2(20),
  NM_ZIP              NVARCHAR2(10),
  NM_COUNTRY          NVARCHAR2(30),
  ID_CREDITCARDTYPE   NUMBER(10)                NOT NULL,
  NM_BANKNAME         NVARCHAR2(40),
  NM_LASTFOURDIGITS   VARCHAR2(4 BYTE)          NOT NULL,
  NM_EXPDATE          VARCHAR2(20 BYTE)         NOT NULL,
  ID_EXPDATEF         NUMBER(10)                NOT NULL,
  NM_STARTDATE        VARCHAR2(20 BYTE),
  NM_ISSUERNUMBER     VARCHAR2(20 BYTE),
  NM_CARDID           NVARCHAR2(4),
  NM_CARDVERIFYVALUE  NVARCHAR2(4),
  NM_PCARD            NVARCHAR2(1)
);
...
-- Fix guid format 
update t_ps_creditcard2
set 
	id_payment_instrument = substr(id_payment_instrument,1,8)||'-'||
                            substr(id_payment_instrument,9,4)||'-'||
                            substr(id_payment_instrument,13,4)||'-'||
                            substr(id_payment_instrument,17,4)||'-'||
                            substr(id_payment_instrument,20);

create table countryMap
(
    shortName varchar2(10),
    enumName varchar2(255)
);
...
--lock table T_PS_CREDITCARD in exclusive mode;

INSERT INTO t_ps_payment_instrument
            (id_payment_instrument, n_payment_method_type, nm_account_number,
             nm_first_name, nm_middle_name, nm_last_name, nm_address1,
             nm_address2, nm_city, nm_state, nm_zip, id_country)
   SELECT tpc.id_payment_instrument, ccenum.id_enum_data, nm_ccnum,
          CASE
             WHEN INSTR (nm_customer, ' ') > 0
                THEN SUBSTR (nm_customer,
                             0,
                             INSTR (nm_customer, ' ')
                            )
             ELSE nm_customer
          END nm_first_name,
          CASE
             WHEN INSTR (nm_customer, ' ', 1, 2) > 0
                THEN SUBSTR (nm_customer,
                             INSTR (nm_customer, ' ') + 1,
                               INSTR (nm_customer, ' ', 1, 2)
                             - INSTR (nm_customer, ' ')
                            )
             ELSE N' '
          END nm_middle_name,
          CASE
             WHEN INSTR (nm_customer, ' ') > 0
                THEN CASE
                       WHEN INSTR (nm_customer, ' ', 1, 2) > 0
                          THEN SUBSTR (nm_customer,
                                       INSTR (nm_customer, ' ', 1, 2) + 1
                                      )
                       ELSE SUBSTR (nm_customer, INSTR (nm_customer, ' ') + 1)
                    END
             ELSE nm_customer
          END nm_last_name,
          nm_address, 
     ...
     FROM t_ps_creditcard2 tpc INNER JOIN countrymap ON nm_country = shortname
          INNER JOIN t_enum_data countryenum ON enumname = countryenum.nm_enum_data
          , t_enum_data ccenum
    WHERE ccenum.nm_enum_data = 'metratech.com/paymentserver/PaymentType/Credit Card';
...
update %%NETMETER%%.t_payment_instrument 
set (id_priority) = (select tmpId
from
    (select id_payment_instrument, 
            id_priority, 
            ROW_NUMBER() over (partition by id_acct order by id_priority) as tmpId 
...
ALTER TABLE T_PS_CREDITCARD RENAME TO T_PS_CREDITCARD_OLD;
