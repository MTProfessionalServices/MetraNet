...
/* todo: we need error handling code to detect when the ancestor does    not exist at the time interval!!    populate t_account_ancestor (no bitemporal data)    */
   ...
   /* Commenting out as a performance bottleneck
...
if p_status < 0 then
 ...
 or p_id_ancestor is not null) AND p_hierarchy_movedate is not null then
  ...
  MoveAccount(ancestorID,AccountID,p_hierarchy_movedate,p_enforce_same_corporation,p_status,p_old_id_ancestor_out,p_ancestor_type, p_acc_type);
  if p_status <> 1 then
   return;
  end if;
 end if;
...
CREATE OR REPLACE procedure UpsertDescription(
...
begin
 /* mark the successful rows as analyzed.*/
    /* Check to see if the state of the data in the rerun table has changed*/
   v_renalyze := NULL;  

   select instr(p_table_name, 't_rerun_session_') INTO v_renalyze from dual; 
   IF (v_renalyze != 0 ) THEN
   v_query := 'delete from ' || p_table_name || 
			' where id_source_sess not in  (select id_source_sess from ' || p_table_name || ' rr inner join t_acc_usage au on rr.id_source_sess = au.tx_UID)';
