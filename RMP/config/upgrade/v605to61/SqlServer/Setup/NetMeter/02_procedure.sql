...
PRINT N'Create [dbo].[GetPaymentInfo]'

USE [NetMeter]
GO

/****** Object:  StoredProcedure [dbo].[GetPaymentInfo]    Script Date: 03/31/2010 16:20:28 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetPaymentInfo]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetPaymentInfo]
GO
...
@id_acc int,                    -- account
@amount numeric(18, 6) OUTPUT,  -- last invoice amount - payments
...
@last_payment numeric(18,6) OUTPUT, 
...
DECLARE
  ...
  -- if we ever need to inclcude adjustments, will have to use product view table.
  SELECT
   @total_payments = SUM(au.Amount)
  FROM t_acc_usage au
   INNER JOIN t_pv_Payment p on p.id_sess = au.id_sess
...
/**************************************************************/
/*   RMP\Config\Queries\DBINstall\ProductCatalog\Queries.xml          */
/**************************************************************/

PRINT N'Create [dbo].[MTEndOfDay]'


IF EXISTS (SELECT name,type FROM sysobjects 
         WHERE name = 'MTEndOfDay' AND type = 'FN')
   DROP FUNCTION MTEndOfDay
...
CREATE function MTEndOfDay(@indate as datetime) returns datetime
	...
	if(DATEPART(d, @indate) != 1 OR DATEPART(m, @indate) != 1 OR DATEPART(yyyy, @indate) != 2038)
		set @retval =
		DATEADD(s,-1,
		DATEADD(d,1,	
...
/**************************************************************/
/*   RMP\Config\Queries\DBINstall\BusinessEntity\Queries.xml          */
/**************************************************************/

PRINT N'Create [dbo].[CreateLocalizedEntry]'


IF EXISTS (SELECT name,type FROM sysobjects 
         WHERE name = 'CreateLocalizedEntry' AND type = 'P')
   DROP PROCEDURE CreateLocalizedEntry
...
/**************************************************************/
/*   RMP\Config\Queries\DBINstall\AccHierarchies\Queries.xml          */
/**************************************************************/

PRINT N'Create [dbo].[BatchAccHierarchyCreation]'

IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'BatchAccHierarchyCreation' AND type = 'P')
   DROP PROCEDURE BatchAccHierarchyCreation

GO

create procedure BatchAccHierarchyCreation
(
...
begin
  ...
  exec sp_executesql @sql
    ...
    -- Do it once where the payer exists in t_account_ancestor
          INSERT INTO t_account_ancestor (id_ancestor, id_descendent, num_generations, vt_start, vt_end, tx_path)
              ...
              INNER JOIN #tmpAccountBatch tmp ON tmp.id_ancestor_out = anc.id_descendent
              WHERE  tmp.status IS NULL
                ...
                AND ((CASE WHEN ((anc.vt_start IS NOT NULL AND anc.vt_start > tmp.hierarchy_end)
                              OR (tmp.hierarchy_start IS NOT NULL AND tmp.hierarchy_start > anc.vt_end))
                            THEN 0 ELSE 1 END) = 1)
    AND EXISTS (select id_descendent from t_account_ancestor aa where tmp.id_ancestor_out = aa.id_descendent)
            UNION ALL
    ...
    AND EXISTS (select id_descendent from t_account_ancestor aa where tmp.id_ancestor_out = aa.id_descendent)
            UNION ALL 
              ...
              FROM   #tmpAccountBatch tmp
              WHERE  tmp.status IS NULL
    AND EXISTS (select aa.id_descendent from t_account_ancestor aa where tmp.id_ancestor_out = aa.id_descendent)
    -- And then do it again for those #tmpAccountBatch records whose id_payer is defined in #tmpAccountBatch
  INSERT INTO t_account_ancestor (id_ancestor, id_descendent, num_generations, vt_start, vt_end, tx_path)
              -- Add records for my parent's ancestors.
              -- Note that these will be one level deeper for me then for my parent.
              SELECT anc.id_ancestor, tmp.id_account, anc.num_generations + 1,
              ...
              WHERE  tmp.status IS NULL
                ...
                AND ((CASE WHEN ((anc.vt_start IS NOT NULL AND anc.vt_start > tmp.hierarchy_end)
                              OR (tmp.hierarchy_start IS NOT NULL AND tmp.hierarchy_start > anc.vt_end))
                            THEN 0 ELSE 1 END) = 1)
    AND NOT EXISTS (select aa.id_descendent from t_account_ancestor aa where tmp.id_account = aa.id_descendent)
            UNION ALL
              -- Add the record for my parent.
              SELECT tmp.id_ancestor_out, tmp.id_account, 1, tmp.hierarchy_start, tmp.hierarchy_end,
                    CASE WHEN (anc.id_descendent = 1 OR anc.id_descendent = -1) THEN
...
--          FROM t_account_ancestor anc
--          INNER JOIN #tmpAccountBatch tmp WITH(READCOMMITTED) ON anc.id_descendent = tmp.id_ancestor_out
--          WHERE  tmp.status IS NULL
--            -- The next AND clause used to be:
--            --   AND (dbo.OverlappingDateRange(anc.vt_start, anc.vt_end, tmp.hierarchy_start, tmp.hierarchy_end) = 1))
--            AND ((CASE WHEN ((anc.vt_start IS NOT NULL AND anc.vt_start > tmp.hierarchy_end)
