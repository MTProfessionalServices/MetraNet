...
select
	id_payment_instrument,
	case 
		 when EXISTS(select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/paymentserver/CreditCardType/Visa' and id_enum_data = id_creditcardtype) then (select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/CreditCardType/Visa')
		 when EXISTS(select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/paymentserver/CreditCardType/MasterCard' and id_enum_data = id_creditcardtype) then (select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/CreditCardType/Master Card')
		 when EXISTS(select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/paymentserver/CreditCardType/American Express' and id_enum_data = id_creditcardtype) then (select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/CreditCardType/American Express')
		 when EXISTS(select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/paymentserver/CreditCardType/Discover' and id_enum_data = id_creditcardtype) then (select id_enum_data from t_enum_data where nm_enum_data = 'metratech.com/CreditCardType/Discover')
	end,
	nm_expdate,
	id_expdatef,
	nm_startdate,
...
from
	t_ps_creditcard pscc
where
	pscc.id_payment_instrument = %%NETMETER%%..t_payment_instrument.id_payment_instrument
	and
	%%NETMETER%%..t_payment_instrument.id_acct is NULL
GO

IF EXISTS(select nm_current from %%NETMETER%%..t_current_id where nm_current = 'temp_acc_id')
	DELETE FROM %%NETMETER%%..t_current_id where nm_current = 'temp_acc_id'
GO

-- Insert new record into t_current_id in NetMeter based on value with same name in MetrPay
-- since MetraPay won't be using it anymore.  This will prevent collisions when both versions are running
INSERT INTO %%NETMETER%%..t_current_id (id_current, nm_current)
select
	id_current, 
	'temp_acc_id'
from
