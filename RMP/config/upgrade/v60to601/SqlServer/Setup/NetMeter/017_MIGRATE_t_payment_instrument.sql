...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[tmp_t_payment_instrument]') AND type in (N'U'))
DROP TABLE [dbo].[tmp_t_payment_instrument]
GO

IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[t_payment_instrument_old]') AND type in (N'U'))
BEGIN

	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[t_payment_instrument]') AND type in (N'U'))
		DROP TABLE [dbo].[t_payment_instrument]

	-- Restore original table
	EXECUTE sp_rename N'dbo.t_payment_instrument_old', N't_payment_instrument', 'OBJECT' 
END
GO
...
(
	...
	nm_city nvarchar(20) NOT NULL,
	nm_state nvarchar(20) NULL,
...
select * into t_payment_instrument_old 
from t_payment_instrument with(HOLDLOCK TABLOCKX)
GO

IF EXISTS(SELECT * FROM dbo.t_payment_instrument)
	 EXEC('INSERT INTO dbo.tmp_t_payment_instrument (id_payment_instrument, id_acct, n_payment_method_type, nm_truncd_acct_num, id_creditcard_type, n_account_type, nm_exp_date, nm_exp_date_format, nm_first_name, nm_middle_name, nm_last_name, nm_address1, nm_address2, nm_city, nm_state, nm_zip, id_country, id_priority, n_max_charge_per_cycle, dt_created)
		SELECT CONVERT(nvarchar(72), id_payment_instrument), id_acct, 
		ccEnum.id_enum_data,
		nm_truncd_acct_num, , 
		case when n_creditcard_type = 1 then (select id_enum_data from t_enum_data where nm_enum_data = ''metratech.com/CreditCardType/Visa'')
			 when n_creditcard_type = 2 then (select id_enum_data from t_enum_data where nm_enum_data = ''metratech.com/CreditCardType/Master Card'')
			 when n_creditcard_type = 3 then (select id_enum_data from t_enum_data where nm_enum_data = ''metratech.com/CreditCardType/American Express'')
			 when n_creditcard_type = 4 then (select id_enum_data from t_enum_data where nm_enum_data = ''metratech.com/CreditCardType/Discover'')
		end id_creditcard_type,
		n_account_type, nm_exp_date, nm_exp_date_format, 
		case when CHARINDEX('' '', nm_customer_name) > 0 then
		substring(nm_customer_name, 0, CHARINDEX('' '', nm_customer_name))
		else nm_customer_name end as nm_first_name,
		case when CHARINDEX('' '', nm_customer_name) > 0 and
			CHARINDEX('' '', nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1) > 0 then
				substring(nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1, CHARINDEX('' '', nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1) - (CHARINDEX('' '', nm_customer_name) + 1))
		else '''' end as nm_middle_name,
		case when CHARINDEX('' '', nm_customer_name) > 0 then
		case when CHARINDEX('' '', nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1) > 0 then
			substring(nm_customer_name, CHARINDEX('' '', nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1) + 1, len(nm_customer_name))
		else
			substring(nm_customer_name, CHARINDEX('' '', nm_customer_name) + 1, len(nm_customer_name))
		end
		else
			nm_customer_name end as nm_last_name,
		nm_address1, nm_address2, nm_city, nm_state, nm_zip, countryEnum.id_enum_data, id_priority, n_max_charge_per_cycle, 
		dt_created FROM
			 dbo.t_payment_instrument WITH (HOLDLOCK TABLOCKX)
			 inner join
...
-- Update priority numbers based on date created
update t_payment_instrument set id_priority = tmpId
from
	(select 
		id_payment_instrument, 
		ROW_NUMBER() over (partition by id_acct order by dt_created) as tmpId 
	 from t_payment_instrument) t
where
	t_payment_instrument.id_payment_instrument = t.id_payment_instrument
GO

IF EXISTS (select * from sys.Objects where object_id = object_id('PK_t_payment_instrument'))
BEGIN
	ALTER TABLE t_payment_instrument DROP CONSTRAINT PK_t_payment_instrument
END
GO
...
EXECUTE sp_rename N'dbo.tmp_t_payment_instrument', N't_payment_instrument', 'OBJECT' 
GO

ALTER TABLE t_payment_instrument ADD CONSTRAINT
PK_t_payment_instrument PRIMARY KEY CLUSTERED 
(
	id_payment_instrument
