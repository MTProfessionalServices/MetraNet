<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/PageExt.master" AutoEventWireup="true"  CodeFile="AccountBillManager.aspx.cs" Inherits="Relationships_AccountBillManager" meta:resourcekey="PageResource1" Culture="auto" UICulture="auto" %>
<%@ Register Assembly="MetraTech.UI.Controls" Namespace="MetraTech.UI.Controls" TagPrefix="MT" %>

<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <MT:MTFilterGrid ID="MTFilterGrid1" runat="server" ExtensionName="Core" 
    TemplateFileName="AccountBillManager.xml" ButtonAlignment="Center" 
  ...
  <script language="javascript" type="text/javascript">
    ...
    function onRemove_<%=MTFilterGrid1.ClientID %>()
    {
      ...
      if (managerIds.length == 0)
        ...
        // do ajax request
        Ext.Ajax.request({
        params: {ids: managerIds},
        url: 'AjaxServices/RemoveAccountBillManager.aspx',
        scope: this,
        disableCaching: true,
    ...
    {
      getFrameMetraNet().getSelection('ProcessSelectedAccounts', 'DropGrid');
    }

    function GetManagerIds()
    {
      var mgrRecords = grid_<%= MTFilterGrid1.ClientID %>.getSelectionModel().getSelections();
      var managerIds = "";
      for(var i=0; i < mgrRecords.length; i++)
      {
        if(i > 0)
        {
          managerIds += ",";
        }
        managerIds += mgrRecords[i].data.AdminID;
      }
      return managerIds;
    }
    ...
    function ProcessSelectedAccounts(ids, records, target)
    {
      ...
      // do ajax request
        Ext.Ajax.request({
        params: {ids : ids},
        url: 'AjaxServices/AddAccountBillManager.aspx',
        scope: this,
        disableCaching: true,
        callback: function (options, success, response) {
        var responseJSON = Ext.decode(response.responseText);
