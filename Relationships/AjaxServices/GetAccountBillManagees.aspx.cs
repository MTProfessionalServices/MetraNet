...
public partial class Relationships_AjaxServices_GetAccountBillManagees : MTListServicePage
{
  ...
  {
    using (new HighResolutionTimer("GetAccountBillManagees", 5000))
    {
      ...
      try
      {
        ...
        items.Filters.Add(new MTFilterElement("AdminID", MTFilterElement.OperationType.Equal, UI.Subscriber.SelectedAccount._AccountID));
        SetPaging(items);
        SetSorting(items);
        SetFilters(items);
