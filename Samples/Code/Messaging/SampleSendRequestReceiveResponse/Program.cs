  ...
  class Program
  {
    static void Main(string[] args)
    {
      ...
      for (int i = 0; i < numberOfRequestsToSend; i++)
      {
        ...
        string xmlBaseMetraTechRequest = "<Request NeedsTimeout=\"{0}\"><MessageType>{1}</MessageType><Body>{2}</Body></Request>";
        
        //Set the type specific custom request properties
        string xmlMyRequestProperties = @"<AccountIdentifier>ABC</AccountIdentifier>
        ...
        string xmlMyMetraTechFormattedRequest = string.Format(xmlBaseMetraTechRequest, "true", "TestMessageType", xmlMyRequestProperties);
      ...
      Console.WriteLine("Sent {0} in {1} ({2:0}/second)", numberOfRequestsToSend, (requestsEndTime - requestsStartTime).TotalSeconds, requestsPerSecond);
      ...
      if (processResponses)
      {
        ...
        while (true)
        {
          if (rabbitConsumer.Queue.Dequeue(timeoutMilliseconds, out result))
          {
            ...
            if (metraTechReturnCode.ToLower().CompareTo("timeout") == 0)
            {
              ...
              Console.WriteLine("[Timeout Received] Request with correlation id {0} has timed out, although it is still being processed", messageInEnvelope.BasicProperties.CorrelationId);
              Console.ResetColor();
            ...
            {
              Console.ForegroundColor = ConsoleColor.Yellow;
              Console.WriteLine("[Warning] Received message with id {0} that was not in list of messages we sent", messageInEnvelope.BasicProperties.CorrelationId);
              Console.ResetColor();
              continue;
            }
            else
            ...
            if (metraTechReturnCode.ToLower().CompareTo("success") != 0)
            {
              ...
              Console.WriteLine("Request with correlation id {0} has not returned success: {1}", messageInEnvelope.BasicProperties.CorrelationId, metraTechReturnCode);
              Console.WriteLine("Response was [{0}]", xmlMessage);
              Console.ResetColor();
            }
            else
            {
        ...
        Console.WriteLine("Received {0} in {1} ({2:0}/second)", responseCount, (responsesEndTime - responsesStartTime).TotalSeconds, responsesPerSecond);
    ...
    private static void DisplayUsage()
    {
      ...
      Console.WriteLine("  SampleSendRequestReceiveResponse [Parameters]");
      Console.WriteLine("");
      Console.WriteLine("Parameters:");
      Console.WriteLine("--NumberToSend=<integer>    Number of requests to send");
      ...
      Console.WriteLine("--processResponses+         Check responses after sending request. This is the default.");
      Console.WriteLine("--processResponses-         Do not check responses, just send requests");
      Console.WriteLine("");
