<%@ Control Language="C#" AutoEventWireup="true" Inherits="UserControls_SiteTimeout" CodeFile="SiteTimeout.ascx.cs" %>
...
<script type="text/javascript">
  ...
  onTimeout = function() {
      location.href = "<%=Request.ApplicationPath%>/Logout.aspx";
  };

  alive = function(btn) {
      Ext.Ajax.request({
              url: '<%=Request.ApplicationPath%>/AjaxServices/GetGUID.aspx',
              scope: this,
              disableCaching: true,
  ...
  showWarningMsg = function () {
    Ext.MessageBox.show({
      ...
      msg: '<asp:Localize meta:resourcekey="SessiontTimeoutMsg1" runat="server">Your session is about to time out as a security measure.</asp:Localize>'
           ...
           + '<span id="timeoutMsg"><asp:Localize meta:resourcekey="SessionTimeoutChangingMessage" runat="server">5 minutes and 0 seconds</asp:Localize></span>.',
