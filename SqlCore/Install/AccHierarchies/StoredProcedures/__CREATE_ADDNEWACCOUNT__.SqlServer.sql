  ...
  if (@p_corporate_account_id is null)
   set @p_corporate_account_id = @accountID
	...
	BEGIN
		...
		SELECT @sessionId = id_current FROM t_current_id WITH(UPDLOCK) WHERE nm_current = 'id_template_session'
		...
		execute ApplyAccountTemplate @templateId, @sessionId, @p_systemdate, NULL, NULL, 'N', 'N', NULL, NULL, @accountID
