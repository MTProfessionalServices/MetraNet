...
BEGIN
EXECUTE IMMEDIATE 'drop table tmp_corps' ;
EXECUTE IMMEDIATE 'drop table tmp_accs';
EXECUTE IMMEDIATE 'drop table all_rcs';
EXECUTE IMMEDIATE 'drop table all_rcs_linked';
EXECUTE IMMEDIATE 'drop table all_rcs_by_month';
EXECUTE IMMEDIATE 'drop table sum_rcs_by_month';
EXECUTE IMMEDIATE 'drop table tmp_fx';
EXECUTE IMMEDIATE 'drop table tmp_unrooted';
EXECUTE IMMEDIATE 'drop table my_unrooted';
EXECUTE IMMEDIATE 'drop table tmp_all_customers';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
      END IF;
END;
EXECUTE IMMEDIATE 'create global temporary table tmp_corps(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table tmp_accs(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table all_rcs(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table all_rcs_linked(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table all_rcs_by_month(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table sum_rcs_by_month(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table tmp_fx(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table tmp_unrooted(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table my_unrooted(x clob) on commit delete rows';
EXECUTE IMMEDIATE 'create global temporary table tmp_all_customers(x clob) on commit delete rows';

create global temporary table tmp_corps(x clob) on commit delete rows;
...
create global temporary table all_rcs_by_month(x clob) on commit delete rows;
...
create global temporary table my_unrooted(x clob) on commit delete rows;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Flushing datamart tables');
end if;
/*
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'Customer')) then 
  EXECUTE IMMEDIATE 'DROP TABLE Customer'; 
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'SalesRep')) then 
  EXECUTE IMMEDIATE 'DROP TABLE SalesRep';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'SubscriptionsByMonth')) then 
  EXECUTE IMMEDIATE 'DROP TABLE SubscriptionsByMonth';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'Subscription')) then 
  EXECUTE IMMEDIATE 'DROP TABLE Subscription';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'SubscriptionPrice')) then
  EXECUTE IMMEDIATE 'DROP TABLE SubscriptionPrice';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'SubscriptionUnits')) then 
EXECUTE IMMEDIATE 'DROP TABLE SubscriptionUnits';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'SubscriptionSummary')) then 
  EXECUTE IMMEDIATE 'DROP TABLE SubscriptionSummary';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'Counters')) then 
  EXECUTE IMMEDIATE 'DROP TABLE Counters';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'CurrencyExchangeMonthly')) then 
  EXECUTE IMMEDIATE 'DROP TABLE CurrencyExchangeMonthly';
end if;
IF (EXISTS (SELECT 1 FROM user_tables WHERE TABLE_SPACE = UPPER('AnalyticsDataMart') AND  TABLE_NAME = 'ProductOffering')) then
  EXECUTE IMMEDIATE 'DROP TABLE ProductOffering';
end if;
*/
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Cleaning out temporary tables');
end if;
/*
IF EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('tmp_unrooted')) then EXECUTE IMMEDIATE 'drop table tmp_unrooted';end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('tmp_all_customers')) then EXECUTE IMMEDIATE 'drop table tmp_all_customers'; end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('tmp_corps')) then EXECUTE IMMEDIATE 'drop table tmp_corps'; end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('tmp_accs')) then EXECUTE IMMEDIATE 'drop table tmp_accs'; end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('all_rcs')) then EXECUTE IMMEDIATE 'drop table all_rcs'; end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('all_rcs_linked')) then EXECUTE IMMEDIATE 'drop table all_rcs_linked';end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('all_rcs_by_month')) then EXECUTE IMMEDIATE 'drop table all_rcs_by_month';end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('sum_rcs_by_month')) then EXECUTE IMMEDIATE 'drop table sum_rcs_by_month';end if;
IF  EXISTS(select 1 FROM user_tables where TBALE_NAME= UPPER('tmp_fx')) then EXECUTE IMMEDIATE 'drop table tmp_fx';end if;
*/
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating Customers DataMart');
end if;

/* TODO: info */
/* TODO: run incrementally instead of drop/recreate each time for usage etc. */
/* customers datamart */
EXECUTE IMMEDIATE 'create global temporary table root_accts on commit preserve rows as
(
...
EXECUTE IMMEDIATE 'create global temporary table  tmp_corps on commit preserve rows as
(select
...
select  count(1) into l_count from tmp_corps ;

if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Found Corporate Accounts: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;

EXECUTE IMMEDIATE 'create global temporary table  my_gens on commit preserve rows as
(
	select
...
EXECUTE IMMEDIATE 'create global temporary table tmp_accs on commit preserve rows as 
(select
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Found Corporate Hierarchy Accounts: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
EXECUTE IMMEDIATE 'create global temporary table root_accs on commit preserve rows
as
(
	select
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Added Non-Corporate Hierarchy Accounts, Total Now: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;

/* non-corporate nodes without hierarchy */
EXECUTE IMMEDIATE 'insert into tmp_accs (id_ancestor, id_descendent)
select
a.id_acc, a.id_acc
from t_account a with(nolock)
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Added Non-Corporate Non-Hierarchy Accounts, Total Now: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
/* unrooted nodes */
EXECUTE IMMEDIATE 'create global temporary table tmp_unrooted on commit preserve rows as
select
...
and b.id_descendent is null'
;
EXECUTE IMMEDIATE 'create global temporary table  my_unrooted on commit preserve rows as
(
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Added Non-Rooted Accounts, Total Now: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;

select id_enum_data into billTo from t_enum_data  where nm_enum_data = 'metratech.com/accountcreation/contacttype/bill-to';
EXECUTE IMMEDIATE 'create global temporary table  tmp_all_customers on commit preserve rows as
select
...
select count(1) into l_count from tmp_all_customers;

if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Reducing duplicate aliases from total: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Customers: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating SalesRep DataMart');
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Sales Reps: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating CurrencyExchangeMonthly DataMart');
end if;

/* TODO: change name of table */
EXECUTE IMMEDIATE 'select
v_nm_instance as InstanceId,
NVL(eff.dt_start, mtmindate()) as StartDate,
NVL(eff.dt_end, MTMaxDate()) as EndDate,
...
inner join t_pt_CurrencyExchangeRates pt on pt.id_sched = rs.id_sched and pt.tt_end = ' + MTMaxDate() +
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Currency Exchange Rates: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating SubscriptionsByMonth DataMart');
end if;
EXECUTE IMMEDIATE 'create global temporary table  my_pis on commit preserve rows as
 as
(
select distinct
sub.id_sub,
...
inner join t_gsub_recur_map grm with(nolock) on grm.id_prop = tr.id_prop and grm.tt_end = mtmaxdate()
where 1=1
)';
EXECUTE IMMEDIATE 'create global temporary table  all_rcs on commit preserve rows as
select
*
into all_rcs
from (
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Found RCs: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Created index for RC linkage');
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Found Linked RCs: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Created index for linked RCs');
end if;
EXECUTE IMMEDIATE 'create global temporary table all_rcs_by_month on commit preserve rows as 
select
rcs.InstanceId,
rcs.SubscriptionId,
...
inner join spt_values months on months.type=''P'' and months.number between 0 and DATEDIFF(month,rcs.startdate, rcs.enddate)
where 1=1'
;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Found RCs by month: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;

EXECUTE IMMEDIATE 'create global temporary table sum_rcs_by_month on commit preserve rows as 
select
rcs.InstanceId,
rcs.SubscriptionId,
rcs.PriceableItemTemplateId,
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Summarized RCs by month: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Created index for summarized subscriptions');
end if;

EXECUTE IMMEDIATE 'create index idx_tmp_fx_rate on tmp_fx (InstanceId,SourceCurrency,TargetCurrency,StartDate,EndDate)';

if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Debug', 'Created index for exchange rates');
end if;
...
EXECUTE IMMEDIATE 'create global temporary table SubscriptionsByMonth on commit preserve rows as 
select
cMonth.InstanceId,
...
NVL(pMonth.TotalAmount,0)*(case when v_nm_currency <> cMonth.Currency then exc.ExchangeRate else 1.0 end) as MRRBasePrimaryCurrency,
0 as MRRRenewal,
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Subscriptions by month: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating SubscriptionSummary DataMart');
end if;

EXECUTE IMMEDIATE 'create global temporary table SubscriptionSummary on commit preserve rows as 
select
mrr.InstanceId,
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Subscription summaries: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating SubscriptionUnits DataMart');
end if;

/* NOTE: this is UDRC's not decision counters */
EXECUTE IMMEDIATE 'create global temporary table SubscriptionUnits on commit preserve rows as 
select
v_nm_instance as InstanceId,
...
where 1=1
and rv.tt_end ='+ mtmaxdate() ;

select count(1) into l_count from SubscriptionUnits;

if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Subscription units: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Generating ProductOffering DataMart');
end if;
EXECUTE IMMEDIATE 'create table ProductOffering as 
select
v_nm_instance as InstanceId,
...
NVL(eff.dt_start, mtmindate()) as EffectiveStartDate,
NVL(eff.dt_end, mtmaxdate()) as EffectiveEndDate,
NVL(avl.dt_start, mtmindate()) as AvailableStartDate,
NVL(avl.dt_end, mtmaxdate()) as AvailableEndDate
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Product Offerings: ' + CAST(NVL(l_count, 0) AS VARCHAR(64)));
end if;
...
if (v_id_run is not null) then
	INSERT INTO t_recevent_run_details (id_run, dt_crt, tx_type, tx_detail) VALUES (v_id_run, v_dt_now, 'Info', 'Finished generating DataMart');
end if;
