        ...
        CREATE OR REPLACE PROCEDURE SequencedInsertGsubRecur (
            p_id_group_sub INT,
            p_id_prop INT,
            p_id_acc INT,
            p_vt_start DATE,
        ...
        BEGIN
            ...
            IF SQL%rowcount <> 1 THEN
                ...
                SELECT COUNT(*) INTO v_cnt FROM T_RECUR WHERE id_prop = p_id_prop;
                IF v_cnt = 0 
                THEN
                    ...
                    RETURN;
                END IF;
                SELECT COUNT(*) INTO v_cnt FROM T_ACCOUNT WHERE id_acc = p_id_acc;
                IF v_cnt = 0 
                THEN
                /* KIOSK_ERR_ACCOUNT_NOT_FOUND */
                    ...
                    RETURN;
                END IF;
                SELECT COUNT(*) INTO v_cnt FROM T_SUB WHERE id_group = p_id_group_sub;
                IF v_cnt = 0 THEN
