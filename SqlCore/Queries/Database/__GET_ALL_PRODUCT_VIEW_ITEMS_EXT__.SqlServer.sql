select %%SELECT_CLAUSE%%, au.id_view ViewID, au.id_sess SessionID, au.amount Amount,
        ...
        case when au.is_implied_tax = 'N' then 	(au.amount + 
        isnull((au.tax_federal), 0.0) + isnull((au.tax_state), 0.0) + isnull((au.tax_county), 0.0) + 
        isnull((au.tax_local), 0.0) + isnull((au.tax_other), 0.0))
      else isnull((au.amount), 0.0) end AmountWithTax, %%INTERVAL_ID%% IntervalID
