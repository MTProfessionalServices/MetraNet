        ...
        sum(nvl(au.tax_federal, 0.0) + nvl(au.tax_state, 0.0) + 
        nvl(au.tax_county, 0.0) + nvl(au.tax_local, 0.0) + 
        nvl(au.tax_other, 0.0)) TaxAmount, sum(nvl(au.amount, 0.0) + nvl(au.tax_federal, 0.0) + 
        nvl(au.tax_state, 0.0) + nvl(au.tax_county, 0.0) + nvl(au.tax_local, 0.0) + 
        nvl(au.tax_other, 0.0)) AmountWithTax, %%ACCOUNT_ID%% AccountID, %%INTERVAL_ID%% IntervalID
