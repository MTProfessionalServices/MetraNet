      ...
      (
      SELECT 
		--__GET_USAGE_DETAIL_INTERVAL_DATAMART__
      au.id_view ViewID, 
      ...
      %%SELECT_CLAUSE%%
      /* ESR-5421 add WITH (FORCESEEK) table hint */
      from t_acc_usage au WITH (FORCESEEK)
      /* ESR-5421 add LOOP join hint */
      inner LOOP join %%TABLE_NAME%% pv on au.id_sess = pv.id_sess and au.id_usage_interval = pv.id_usage_interval            
      left outer join
      (
        select {fn IFNULL((SUM(ChildAdjustments.CompoundPrebillAdjAmt)), 0.0)} + {fn IFNULL((ParentAdjustments.CompoundPrebillAdjAmt), 0.0)} as CompoundPrebillAdjAmt
         ...
         /* ESR-5421 add (FORCESEEK) table hint */
         from t_acc_usage au WITH (FORCESEEK)
         /* ESR-5421 add LOOP join hint */
         inner LOOP join %%TABLE_NAME%% pv_c on au.id_sess = pv_c.id_sess and au.id_usage_interval = pv_c.id_usage_interval
