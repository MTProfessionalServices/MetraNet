...
ViewName = case when instance_desc.id_desc is NULL then template_desc.tx_desc else instance_desc.tx_desc end,
'Product' ViewType,
...
JOIN t_acc_usage au on au.id_acc = %%ID_ACC%% AND au.id_usage_interval = %%INTERVAL%% AND au.id_pi_template is not NULL
...
JOIN t_description template_desc on template_props.n_display_name = template_desc.id_desc AND template_desc.id_lang_code = %%LANG_ID%%
LEFT OUTER JOIN t_base_props tb_instance on tb_instance.id_prop = au.id_pi_instance 
LEFT OUTER JOIN t_description instance_desc on instance_desc.id_desc = tb_instance.n_display_name  AND instance_desc.id_lang_code = %%INTERVAL%%
...
LEFT OUTER JOIN t_description on t_description.id_desc = tb_po.n_display_name AND t_description.id_lang_code = %%LANG_ID%%
where
