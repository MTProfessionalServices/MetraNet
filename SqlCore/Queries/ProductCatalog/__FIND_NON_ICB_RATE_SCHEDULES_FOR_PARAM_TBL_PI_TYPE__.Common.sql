        ...
        select rs.id_sched, bpl.nm_name as pl_nm_name, bpi.nm_name as pi_nm_name, brs.nm_desc as rs_nm_desc,
            ed.n_begintype, ed.dt_start, ed.n_beginoffset, ed.n_endtype, ed.dt_end, ed.n_endoffset
        from t_rsched rs
		...
		join t_pi_template bpitemplate on bpitemplate.id_template = bpi.id_prop
        where pl.n_type = 1 and rs.id_pt = %%PT%% and bpitemplate.id_pi = %%PI%%
