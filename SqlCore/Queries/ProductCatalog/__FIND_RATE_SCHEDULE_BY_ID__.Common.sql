        ...
        UNION ALL
        select t_rsched_pub.id_sched, t_rsched_pub.id_pt, t_rsched_pub.id_eff_date, t_rsched_pub.id_pricelist, t_rsched_pub.id_pi_template,
        t_vw_base_props.n_desc, t_vw_base_props.nm_name, t_vw_base_props.nm_desc,
        te.n_begintype, te.dt_start, te.n_beginoffset,
        te.n_endtype, te.dt_end, te.n_endoffset,
        ts.id_sub,ts.id_acc,
        ...
        from t_rsched_pub
        JOIN t_vw_base_props ON t_rsched_pub.id_sched = t_vw_base_props.id_prop and t_vw_base_props.id_lang_code = %%ID_LANG%%
        JOIN t_effectivedate te ON t_rsched_pub.id_eff_date = te.id_eff_date
        JOIN t_pricelist tp on tp.id_pricelist = t_rsched_pub.id_pricelist
        LEFT OUTER JOIN t_pl_map map on map.id_pricelist = tp.id_pricelist AND tp.n_type = 1 AND map.id_sub is not NULL
        LEFT OUTER JOIN t_sub ts on ts.id_sub = map.id_sub
        where t_rsched_pub.id_sched = %%ID%%
