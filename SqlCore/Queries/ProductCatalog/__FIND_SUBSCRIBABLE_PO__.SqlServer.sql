          ...
          (select tx_desc as nm_display_name from t_description	where id_desc = t_base_props.n_display_name and id_lang_code=%%ID_LANG%%) nm_display_name,
						...
						from  
							...
							FROM
									...
									(
										...
										inner join t_payment_redirection pr ON pr.id_payee = %%ACC_ID%%
                ...
                LEFT OUTER JOIN t_aggregate on t_aggregate.id_prop = t_pl_map.id_pi_template  OR t_aggregate.id_prop = t_pl_map.id_pi_instance
                 ...
                 (rc.tx_cycle_mode = 'EBCR' AND dbo.CheckEBCRCycleTypeCompatibility(rc.id_cycle_type, cycleType.id_cycle_type) = 0) OR
					...
					t_po.id_po in (select id_po from vw_acc_po_restrictions where id_acc=%%ACC_ID%%)
