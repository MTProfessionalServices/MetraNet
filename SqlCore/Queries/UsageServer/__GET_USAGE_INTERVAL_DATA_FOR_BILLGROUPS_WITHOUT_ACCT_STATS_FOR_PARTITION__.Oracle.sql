/* GET_USAGE_INTERVAL_DATA_FOR_BILLGROUPS_WITHOUT_ACCT_STATS_FOR_PARTITION */              
        ...
        inner join t_billgroup on t_billgroup.id_billgroup = tbm.id_materialization and t_billgroup.id_partition = %%ID_PARTITION%%
				where tbm.tx_type = 'Full' 
				and tbm.tx_status = 'Succeeded'
        and tbm.id_parent_billgroup is null
        union all
				select tbm.id_usage_interval 
				from t_billgroup_materialization tbm
        inner join t_billgroup on t_billgroup.id_billgroup = tbm.id_parent_billgroup and t_billgroup.id_partition = %%ID_PARTITION%%
				where tbm.tx_type = 'Full' 
				and tbm.tx_status = 'Succeeded'
        and tbm.id_parent_billgroup is not null        
			),
			billgroups as
			(
      ...
      left outer join /* unassigned accounts */
          (select id_acc, id_usage_interval
	         ...
	         on bg.id_billgroup = bgm.id_billgroup and bg.id_partition = %%ID_PARTITION%%) unassigned on unassigned.id_acc = aui.id_acc 
      ...
      left outer join /* unassigned accounts */
        ...
        (NVL(accounts.HardClosedUnassignedAcctsCnt,0) + NVL(partition_root_account.HardClosedUnassignedAcctsCnt,0)) HardClosedUnassignedAcctsCnt,
        (NVL(accounts.OpenUnassignedAcctsCnt,0) + NVL(partition_root_account.OpenUnassignedAcctsCnt,0)) OpenUnassignedAcctsCnt
      ...
      left outer join partition_root_account on partition_root_account.id_usage_interval = ui.id_interval
