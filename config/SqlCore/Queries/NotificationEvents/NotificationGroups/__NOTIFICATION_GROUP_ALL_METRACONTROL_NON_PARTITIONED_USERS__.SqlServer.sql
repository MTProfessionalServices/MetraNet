...
/* non-partitioned accounts that have the Application Logon to MOM capability */  
  SELECT 	  	
	prp.id_acc 
...
WHERE 1=1 
  ...
  AND acc.id_acc IS NULL
  
UNION ALL
/* non-partitioned accounts that are assigned a role which includes Application Logon to MOM capability */
SELECT 		
	prp.id_acc  
	,acc.id_acc AS id_Partition
FROM     
