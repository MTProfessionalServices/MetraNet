...
/* accounts that have the Application Logon to MCM capability */
SELECT 		
	prp.id_acc, 
...
/* accounts that are assigned a role which includes Application Logon to MCM capability */
SELECT 		
	prp.id_acc  
	,acc.id_acc AS id_Partition
FROM     
...
WHERE  1=1 
  ...
  AND pr.id_role = (SELECT SUBR.id_role
					...
					where SUBR.id_role = pr.id_role AND ed.nm_enum_data = 'Global/Application/MCM')
