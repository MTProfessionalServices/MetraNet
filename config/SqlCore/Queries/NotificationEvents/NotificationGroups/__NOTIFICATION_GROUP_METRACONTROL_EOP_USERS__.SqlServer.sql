...
FROM     
    ...
    WHERE 1=1
      ...
      AND prp.policy_type = 'A' AND prp.id_acc IS NOT NULL 
UNION
SELECT 
  prp.id_acc,
acc.id_acc AS id_Partition
...
LEFT OUTER JOIN t_account_ancestor accancestor ON accancestor.id_descendent = prp.id_acc AND num_generations = 2
  LEFT OUTER JOIN t_account acc ON acc.id_acc = accancestor.id_ancestor AND acc.id_type = 9
WHERE id_policy IN ( 
                      SELECT id_policy FROM t_policy_role 
                      WHERE id_role IN (
                                          ...
                                          (SELECT                                                 
                                           ...
                                           T_PRINCIPAL_POLICY prp     
                                          INNER JOIN t_capability_instance ci ON prp.ID_POLICY = CI.ID_POLICY    
                                          WHERE 1=1 AND ci.id_cap_type = (select id_cap_type from t_composite_capability_type where tx_progid = 'Metratech.MTAllCapability') AND prp.policy_type = 'A' AND prp.id_acc IS NULL
                                          ) rolemapper
                                        ) 
                 )
