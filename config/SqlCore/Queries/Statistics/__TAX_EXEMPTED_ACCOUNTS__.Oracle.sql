select 
  ...
  concat(mapClient.nm_login, concat(mapClient.nm_space, avi.c_TaxExemptEndDate)) "Unique identifier",
	...
	NVL(avi.c_TaxExemptID,'0') "Tax Exempt ID",
...
WHERE  
   ( ( ( avi.c_MetraTaxHasOverrideBand is not null and (avi.c_MetraTaxOverrideBand = (select id_enum_data  from t_enum_data where nm_enum_data = 'metratech.com/tax/TaxBand/exempt'))) 
   OR 
      ...
      AND  /* if any of the above are true, then it is some how an exempted account therefore validate
           ...
           */ 
        ( (c_TaxExemptEndDate is null) OR (c_TaxExemptEndDate > sysdate )  )
    )
