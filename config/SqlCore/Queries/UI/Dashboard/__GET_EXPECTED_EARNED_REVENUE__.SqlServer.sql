SELECT 
	...
	,c_ProratedDays = CASE 
					WHEN udrc.c_RCIntervalSubscriptionStart IS NOT NULL
						THEN DATEDIFF(DAY, udrc.c_RCIntervalSubscriptionStart, %%START_DATE%%)
					WHEN frc.c_RCIntervalSubscriptionStart IS NOT NULL
						THEN DATEDIFF(DAY, frc.c_RCIntervalSubscriptionStart, %%START_DATE%%)
					WHEN nrc.c_NRCIntervalSubscriptionStart IS NOT NULL
						THEN 1
...
WHERE
	(acr.c_CreditTime < %%START_DATE%% OR nsc.c_IssueTime < %%START_DATE%% OR udrc.c_RCIntervalSubscriptionEnd < %%START_DATE%% OR frc.c_RCIntervalSubscriptionEnd < %%START_DATE%% OR nrc.c_NRCIntervalSubscriptionEnd < %%START_DATE%% OR acc.dt_crt < %%START_DATE%%)
