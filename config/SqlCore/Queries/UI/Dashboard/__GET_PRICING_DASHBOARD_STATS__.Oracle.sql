...
nvl(avg((nvl(m.dt_assigned, sysdate)-m.dt_crt)*60*60*24),0.0) as c_count
from t_message m
where 1=1
and nvl(m.dt_assigned, sysdate) between (SYSDATE - (1/(60*24))) and sysdate
union all
select /* we do not treat resubmits special */
'pipeline_seconds' as c_category,
nvl(avg((m.dt_completed - m.dt_assigned)*60*60*24)),0.0) as c_count
from t_message m
where 1=1
