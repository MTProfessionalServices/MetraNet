select top 20 ss.ProductCode, ss.Month, sum(ss.MRR) as 'MRR', sum(prev.MRR) as 'MRRPrevious', sum(ss.MRR)-sum(prev.MRR) as 'MRRChange',
...
HAVING sum(ss.Subscriptions)-sum(prev.Subscriptions) < 0
ORDER BY sum(ss.Subscriptions)-sum(prev.Subscriptions) asc