...
'Getting Product Offerig ID from product offering,coz once i finished the wizard i need to return
'the Non recurring charge to corresponding product offering.

if (Request.QueryString("ID")="") then
  session("POID") = 0
else
  session("POID")= Request.QueryString("ID")
End if	
'POMODE is 1 means,called from Product offering else called from Service  Charges
if (Request.QueryString("POMode")="") then
  session("POMode") = 0
else
  session("POMode")= Request.QueryString("POMode")
End if	

'See if we should skip selecting an existing and just create a new item
'response.write("Rudi [" & request("CREATENEW") & "]<BR>")
'response.end
