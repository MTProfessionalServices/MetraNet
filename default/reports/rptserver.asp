...
Case "SRCH"
	...
	Case "HTML_FRAME"
		...
		'  find out what page the text is on
...
Case "TOOLBAR_PAGE"
	...
	if session("LastPageNumber") <> "" then
		if CLng(PAGE) > CLng(session("LastPageNumber")) then
			session("CurrentPageNumber") = session("LastPageNumber")
		else
...
SUB ParseQS()
	...
	INCOMPLETE_PAGE = CLng(request.querystring("INCOMPLETE_PAGE"))' Used to specify whether the page is to contain placeholders.
	INCOMPLETE_PAGE_COUNT = CLng(request.querystring("INCOMPLETE_PAGE_COUNT"))' Used to specify whether the page has to contain a total page count.
	PVERSION = CLng(request.querystring("PVERSION"))' Used to indicate the protocol version the viewer is utilizing.
	...
	if request.form("INCOMPLETE_PAGE") <> "" then
		INCOMPLETE_PAGE = CLng(request.form("INCOMPLETE_PAGE"))
	end if
	
	if request.form("INCOMPLETE_PAGE_COUNT") <> "" then
		INCOMPLETE_PAGE_COUNT = CLng(request.form("INCOMPLETE_PAGE_COUNT"))
	end if
	
	if PVERSION = 0 then
		PVERSION = CLng(request.form("PVERSION"))
	end if
...
FUNCTION FillExportOptionsObject(export_fmt_options)
	...
	'Get the export format type value
		...
		Select Case exportDLLName
			Case "U2FWORDW"
				ExportOptions.FormatType = 	CREFTWORDFORWINDOWS + CLng(exportType)
				Response.ContentType = "application/msword"
			Case "U2FRTF"
				ExportOptions.FormatType = 	CREFTRICHTEXT	+ CLng(exportType)
				Response.ContentType = "application/rtf"
			Case "U2FXLS"
				ExportOptions.FormatType = CREFTEXCEL21	+ CLng(exportType)
				Response.ContentType = "application/x-msexcel"
			Case "U2FCR"
				ExportOptions.FormatType = CREFTCRYSTALREPORT	+ CLng(exportType)
