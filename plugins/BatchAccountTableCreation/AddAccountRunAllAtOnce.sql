  ...
  -- TODO:  Create an entry for MT_UNABLE_TO_CREATE_SITE_RECORD, MT_ACCOUNT_ERR_DBERROR and
  --          MT_UNABLE_TO_CREATE_POLICY_RECORD in AccHierarchies_err.mc1.  Update the decimal and
  --          hex values used in this code.
  --        Remove the AddNewAccount and CreateUsageIntervalsForAccount stored procedures when
  --          we no longer provide support for the old plug-in.  All other stored procedures used
  --          by the old AddNewAccount stored procedure are also used in other places.
  ...
  --              %%TMP_TABLE_IDX_NAME%%       - Name of the id_account index on the temporary
  --                                             table.  (ex. "idx_tmp_create_account_385")
  ...
  SELECT @system_datetime = GetUTCDate()
  ...
  -- Step 3d: Add an index to the id_account column.
  CREATE CLUSTERED INDEX idx_tmp_create_account ON tmp_create_account (id_account)
  ...
  SET @CursorVar = CURSOR FORWARD_ONLY STATIC
    FOR SELECT DISTINCT nm_space, langcode FROM tmp_create_account WHERE (status IS NULL)
  OPEN @CursorVar
  ...
  BEGIN
    BEGIN TRAN  -- Needed because the code allows this SQL
      ...
      -- NOTE: The code inside this BEGIN/END block closely realted to
      ...
      INNER JOIN tmp_create_account tmp ON
  ...
  -- Step 7b: Find out whether or not this account's ancestor is a folder.
  UPDATE tmp
  ...
  -- Step 7c: Validate that the ancestor id was successfully resolved,
  ...
  -- Step 7d: Validate that the account is not already in the
  ...
  -- Step 7e: Save the account id as a string.
  UPDATE tmp
  SET    tmp.account_id_as_string = CAST(tmp.id_account AS VARCHAR(50))
  FROM   tmp_create_account tmp
  WHERE  tmp.status IS NULL

  -- Step 7f: Populate t_account_ancestor.
  INSERT INTO t_account_ancestor (id_ancestor, id_descendent, num_generations, vt_start, vt_end, tx_path)
      ...
      INNER JOIN tmp_create_account tmp ON tmp.id_ancestor_out = anc.id_descendent
      WHERE  tmp.status IS NULL
  ...
  -- Step 7g: Update each parent entry so that they indicate they have children.
  UPDATE anc
  SET anc.b_Children = 'Y'
  FROM t_account_ancestor anc
  INNER JOIN tmp_create_account tmp ON anc.id_descendent = tmp.id_account
  ...
  -- Step 8h: You can not have an account without a billable payer.
  --
  ...
  INNER JOIN tmp_create_account tmp ON descendent.id_descendent = tmp.id_payer
  ...
  JOIN tmp_create_account tmp ON his.id_payee = tmp.id_account
  WHERE  tmp.status IS NULL
  ...
  INNER JOIN tmp_create_account tmp ON pp.id_acc = tmp.auth_ancestor
  ...
  --          rule then these accounts must use the same currency
  --          as thier parents.
  UPDATE tmp
