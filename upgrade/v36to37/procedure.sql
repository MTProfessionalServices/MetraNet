...
as
  ...
  exec sp_dboption @strDBName, N'autoclose', N'false'

  

  exec sp_dboption @strDBName, N'bulkcopy', N'false'

  

  exec sp_dboption @strDBName, N'trunc. log', N'true'

  

  exec sp_dboption @strDBName, N'torn page detection', N'true'

  

  exec sp_dboption @strDBName, N'read only', N'false'

  

  exec sp_dboption @strDBName, N'dbo use', N'false'

  

  exec sp_dboption @strDBName, N'single', N'false'

  

  exec sp_dboption @strDBName, N'autoshrink', N'false'

  

  exec sp_dboption @strDBName, N'ANSI null default', N'false'

  

  exec sp_dboption @strDBName, N'recursive triggers', N'false'

  

  exec sp_dboption @strDBName, N'ANSI nulls', N'false'

  

  exec sp_dboption @strDBName, N'concat null yields null', N'false'

  

  exec sp_dboption @strDBName, N'cursor close on commit', N'false'

  

  exec sp_dboption @strDBName, N'default to local cursor', N'false'

  

  exec sp_dboption @strDBName, N'quoted identifier', N'false'

  

  exec sp_dboption @strDBName, N'ANSI warnings', N'false'

  

  exec sp_dboption @strDBName, N'auto create statistics', N'true'

  

  exec sp_dboption @strDBName, N'auto update statistics', N'true'
...
GO
			...
			declare @sql nvarchar(4000)
				...
				*/
				...
				CREATE TABLE #t_av_tables (name varchar(255) NULL)



				-- Insert the IDs that are passed in

				INSERT INTO #t_av_tables 
				...
				-- TODO: Open a cursor here



				-- Delete from t_av_* tables

				PRINT '-- Deleting from t_av_contact --'

				DELETE FROM t_av_contact
				...
				BEGIN
					...
					PRINT 'Cannot delete from t_av_contact table'
				...
				PRINT '-- Deleting from t_av_internal --'

				DELETE FROM t_av_internal
				...
				BEGIN
					...
					PRINT 'Cannot delete from t_av_internal table'
				...
				BEGIN
					...
					PRINT 'Cannot delete from t_MT_ID table'
...
GO
	  ...
	  create proc InsertAuditEvent @id_userid int, @id_event int, @id_entity_type int, @id_entity int, @dt_timestamp datetime, @tx_details nvarchar(4000), @id_identity int OUTPUT as
 	 	--if (@id_parent_sess = -1) 
  	begin
...
GO
		...
		--exec archive @interval=11394,@path='e:\usage',@servername='HARV',@dbname='netmeter36',@result=@result output
		...
		begin
			...
			set @result = '1000002-archive operation failed-->Interval is already archived'
		...
		declare  c1 cursor fast_forward for select distinct id_view from t_acc_usage where id_usage_interval = @interval
		...
		select @str1 = N'create view bcpview as SELECT top 100 percent * FROM ' + @dbname + '..t_acc_usage where id_usage_interval=' + cast (@interval as varchar(10)) + N' order by id_sess'

				exec (@str1)

		--BCP out the data from t_acc_usage
		select @sql1 = 'bcp "' + @dbname + '..bcpview" out "' +  @path + '\t_acc_usage' + cast (@interval as varchar (10)) + '.txt" -c -S' + @servername
		...
		BEGIN
			...
			set @result = '1000003-archive operation failed-->Error in bcp out usage table, check the user permissions'
		...
		BEGIN
			...
			set @result = '1000004-archive operation failed-->Error in bcp out usage table, check the archive directory or hard disk space or database name or servername'
		...
		select @str1 = N'SELECT id_sess into tmp_t_acc_usage FROM ' + @dbname + '..t_acc_usage where id_usage_interval=' + cast (@interval as varchar(10)) + N' order by id_sess'

				exec (@str1)

		select @str1 = N'create unique clustered index idx_tmp_t_acc_usage on tmp_t_acc_usage(id_sess)'
		...
		begin
			...
			select @str1 = N'create view bcpview as SELECT top 100 percent pv.* FROM ' + @dbname + '..' + @tab1 + ' pv inner join 

					tmp_t_acc_usage au on pv.id_sess=au.id_sess order by pv.id_sess'

				exec (@str1)

			--BCP out the data from product view tables

			select @sql1 = 'bcp "' + @dbname + '..bcpview" out "' + @path + '\' + @tab1 + cast (@interval as varchar (10)) + '.txt" -c -S' + @servername
			...
			BEGIN
				...
				set @result = '1000005-archive operation failed-->Error in bcp out the ' + cast(@tab1 as varchar(1000)) + ' product view table, check the hard disk space'
		...
		select @str1 = N'create view bcpview as SELECT top 100 percent * FROM ' + @dbname +'..t_adjustment_transaction where id_usage_interval=' + 
		...
		select @sql1 = 'bcp "' + @dbname + '..bcpview" out "' + @path + '\t_adjustment_transaction' + cast (@interval as varchar (10)) + '.txt" -c -S' + @servername
		...
		BEGIN
			...
			set @result = '1000006-archive operation failed-->Error in bcp out adjustment transaction table, check the hard disk space'
		...
		select @str1 = N'SELECT id_adj_trx into tmp_t_adjustment_transaction FROM ' + @dbname + '..t_adjustment_transaction where id_usage_interval=' + cast (@interval as varchar(10)) + N' order by id_sess'

				exec (@str1)

		select @str1 = N'create unique clustered index idx_tmp_t_adjustment_transaction on tmp_t_adjustment_transaction(id_adj_trx)'
		...
		begin
			...
			select @str1 = N'create view bcpview as SELECT top 100 percent aj.* FROM ' + @dbname + '..' + @var1 

			+ ' aj inner join tmp_t_adjustment_transaction trans on aj.id_adjustment=trans.id_adj_trx'

				exec (@str1)

			select @sql1 = 'bcp "' + @dbname +'..bcpview" out "' + @path + '\' + @var1 + cast (@interval as varchar (10)) + '.txt" -c -S' + @servername
			...
			BEGIN
				...
				set @result = '1000007-archive operation failed-->Error in bcp out ' + cast(@var1 as varchar(1000)) + ' table, check the hard disk space'
		...
		begin tran
			...
			begin
				...
				if exists (select 1 from t_archive where id_interval=@interval and id_view=@var1 and status='D' and tt_end=dbo.mtmaxdate())
				...
				begin
					...
					begin
						...
						set @result = '1000008-archive operation failed-->Error in update t_archive table'
				...
				insert into t_archive values(@interval,@var1,null,'A',@vartime,@maxtime)

				if (@@error <> 0)
				begin

					set @result = '1000009-archive operation failed-->Error in insert t_archive table'
			...
			select @str1 = N'delete FROM ' + @dbname + '..' + @tab1 + ' where id_sess in (select id_sess from ' + @dbname + '..t_acc_usage au where au.id_usage_interval=' 

					+ cast (@interval as varchar (10)) + ' and id_view =' + + cast (@var1 as varchar (5)) +')'
			...
			begin
				...
				set @result = '1000010-archive operation failed-->Error in Delete operation'
			...
			begin
				...
				if exists (select 1 from t_archive where id_interval=@interval and adj_name=@var1 and status='D' and tt_end=dbo.mtmaxdate())
				...
				begin
					...
					begin
						...
						set @result = '1000011-archive operation failed-->Error in update t_archive table'
				...
				insert into t_archive values(@interval,null,@var1,'A',@vartime,@maxtime)

				if (@@error <> 0)

				begin

					set @result = '1000012-archive operation failed-->Error in insert into t_archive table'
			...
			select @str1 = N'delete FROM ' + @dbname + '..' + @var1 +  
			...
			begin
				...
				set @result = '1000013-archive operation failed-->Error in Delete operation'
			...
			select @str1 = N'delete FROM ' + @dbname + '..t_adjustment_transaction where id_usage_interval=' + cast (@interval as varchar(10))
			...
			begin
				...
				set @result = 'Error in Delete from t_adjustment_transaction table'
			...
			begin
				...
				set @result = '1000014-archive operation failed-->Error in create adjustment temp table operation'
			...
			begin
				...
				begin
					...
					set @result = '1000015-archive operation failed-->Error in Update adjustment operation'
			...
			select @str1 = N'delete FROM ' + @dbname + '..t_acc_usage where id_usage_interval=' + cast (@interval as varchar(10))
			...
			begin
				...
				set @result = '1000016-archive operation failed-->Error in Delete t_acc_usage operation'
			...
			SET @SQLStmt = N'IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(''tmp_t_adjustment_transaction'')) drop table tmp_t_adjustment_transaction'

				EXEC sp_executesql @SQLStmt

			SET @SQLStmt = N'IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(''tmp_t_acc_usage'')) drop table tmp_t_acc_usage'

				EXEC sp_executesql @SQLStmt



		set @result = '0-archive operation successful'
...
GO
		...
		--exec dearchive intervalid,'archive directory','databasename','Y',@result output
		...
		begin 
			...
			set @result = '2000007-dearchive operation failed-->check the database name'
		...
		if not exists (select top 1 * from t_archive where id_interval=@interval and status ='A' and tt_end = dbo.mtmaxdate())

		begin

			set @result = '2000001-dearchive operation failed-->Interval is not archived'

			return

		end

		if exists (select top 1 * from t_acc_usage where id_usage_interval=@interval)

		begin

			set @result = '2000002-dearchive operation failed-->interval already have usage'
		...
		select  @FileName = 't_acc_usage' + cast (@interval as varchar(10)) + '.txt'
		...
		begin
			...
			set @result = '2000003-dearchive operation failed-->bcp usage file does not exist'

			return

		end

		select  @FileName = 't_adjustment_transaction' + cast (@interval as varchar(10)) + '.txt'
		...
		begin
			...
			set @result = '2000004-dearchive operation failed-->bcp t_adjustment_transaction file does not exist'
		...
		declare  c1 cursor fast_forward for select distinct id_view from t_archive where id_interval = @interval and tt_end = dbo.mtmaxdate()
		...
		begin
			...
			select @filename = @tab1 + cast (@interval as varchar(10)) + '.txt'
			...
			begin
				...
				set @result = '2000005-dearchive operation failed-->bcp ' + cast(@FileName as nvarchar(128)) + ' file does not exist'
		...
		declare  c1 cursor fast_forward for select distinct adj_name from t_archive where id_interval = @interval and tt_end = dbo.mtmaxdate()
		...
		begin
			...
			select @filename = @var1 + cast (@interval as varchar(10)) + '.txt'
			...
			begin
				...
				set @result = '2000006-dearchive operation failed-->bcp ' + cast(@FileName as nvarchar(128)) + ' file does not exist'
		...
		select @sql1 = 'bulk insert ' + @dbname + '..t_acc_usage from ''' + @path + '\t_acc_usage' + cast (@interval as varchar(10)) + '.txt'' WITH 
		...
		begin
			...
			set @result = '2000008-dearchive operation failed-->error in usage bulk insert operation'
		...
		begin
			...
			if exists (select 1 from t_acc_usage where id_pi_template not in ( select id_template from t_pi_template))

			begin

				set @result = '2000009-dearchive operation failed-->id_pi_template key violation'
			...
			if exists (select 1 from t_acc_usage where id_pi_instance not in ( select id_pi_instance from t_pl_map))

			begin

				set @result = '2000010-dearchive operation failed-->id_pi_instance key violation'
			...
			if exists (select 1 from t_acc_usage where id_view not in ( select id_view from t_prod_view))

			begin

				set @result = '2000011-dearchive operation failed-->id_view key violation'
		...
		select @sql1 = 'bulk insert ' + @dbname + '..tmp_t_adjustment_transaction from ''' + @path + '\t_adjustment_transaction' + cast (@interval as varchar(10)) + '.txt'' WITH 
		...
		begin
			...
			set @result = '2000012-dearchive operation failed-->error in adjustment bulk insert operation'
		...
		update trans 
			...
			from tmp_t_adjustment_transaction trans inner join t_acc_usage au on trans.archive_sess=au.id_sess
		...
		begin
			...
			set @result = '2000013-dearchive operation failed-->error in update adjustment transaction'
		...
		--This update is to cover the scenario if post bill adjustments are archived before usage interval and now dearchive before usage interval

		update trans 

			set archive_sess=id_sess,id_sess=null

			from tmp_t_adjustment_transaction trans where not exists (select 1 from t_acc_usage au where au.id_sess=trans.id_sess)
		...
		begin
			...
			set @result = '2000014-dearchive operation failed-->error in update adjustment transaction'
		...
		begin
			...
			select @sql1 = 'bulk insert ' + @dbname + '..' + @tab1 + ' from ''' + @path + '\' + @tab1 + cast (@interval as varchar(10)) + '.txt'' WITH 
			...
			begin
				...
				set @result = '2000015-dearchive operation failed-->error in bulk insert operation for table ' + cast(@tab1 as nvarchar(1000)) 
			...
			begin
				...
				set @result = '2000016-dearchive operation failed-->error in update t_archive'
			...
			insert into t_archive values(@interval,@var1,null,'D',@vartime,@maxtime)

			if (@@error <> 0) 

			begin

				set @result = '2000017-dearchive operation failed-->error in insert t_archive'
		...
		begin
			...
			select @sql1 = 'bulk insert ' + @dbname + '..' + @var1 + ' from ''' + @path + '\' + @var1 + cast (@interval as varchar(10)) + '.txt'' WITH 
			...
			begin
				...
				set @result = '2000018-dearchive operation failed-->error in bulk insert operation for table ' + cast(@var1 as nvarchar(1000))
			...
			begin
				...
				set @result = '2000019-dearchive operation failed-->error in update t_archive'
			...
			insert into t_archive values(@interval,null,@var1,'D',@vartime,@maxtime)

			if (@@error <> 0) 

			begin

				set @result = '2000020-dearchive operation failed-->error in insert t_archive'
