...
'*
'* These functions upgrade MetraTech Platform database from V5.1 to V5.1.1
'*
'******************************************************************************************
...
Const kDBState_DB_51_PRESENT		= "5.1"
...
Public Function Main() ' As Boolean
	...
	if (DBStatus = kDBState_DB_51_PRESENT) then
		' call the upgrade database function to upgrade from 5.1 to 5.1.1
		UpgradeDatabaseSQLFromV51
	else 
...
Function UpgradeDatabaseSQLFromV51
	...
	writelog("*** Upgrading Core Database from v5.1 ... ***")

	writelog("*** Update Database Schema...***")	
	strInFile = strInstallDir & "RMP\config\upgrade\V51to511\v51to511.sql"
	strOutFile = strInstallDir & "RMP\config\upgrade\V51to511\v51to511.txt"
	...
	strInFile = strInstallDir & "RMP\Config\upgrade\V51to511\Procedure.sql"
	strOutFile = strInstallDir & "RMP\Config\upgrade\V51to511\Procedure.txt"
	strFilename = "osql.exe"
	strArguements = " -l 3000 -S " & strDBHostName & " -d " & strDBName & " -U " & strAdminID & " -P " & """" & strAdminPwd & """" & " -i " & strInFile & " -o " & strOutFile
	intstatus = WshShell.Run(strfilename & strArguements,VBHide, true)
	...
	writelog("*** Upgraded Core Database from v5.1 to 5.1.1 successfully ... ***")
