...
VALUES
	VALUES	('6.1.1', getdate(), 'C')

/**************************************************************/
/*   RMP\Config\DBINstall\Queries.xml                         */
/**************************************************************/

PRINT N'Alter [dbo].[t_pending_payment_trans]'
GO      
ALTER  TABLE [dbo].[t_pending_payment_trans] ADD 
    [b_try_dunning] [char](1) NOT NULL DEFAULT '0',
	[b_scheduled] [char](1) NOT NULL DEFAULT '0',
	[dt_create] [datetime] NOT NULL DEFAULT (GetDate())
	


PRINT N'Drop [dbo].[t_sys_upgrade]'

DROP TABLE [dbo].[t_sys_upgrade]

PRINT N'Create [dbo].[t_sys_upgrade]'
...
PRINT N'Insert [dbo].[ t_current_id]'
...
/**************************************************************/
/*   RMP\Config\DBINstall\Auth\Queries.xml                    */
/**************************************************************/

PRINT N'Create [dbo].[t_active_tickets]'

DROP TABLE [dbo].[t_active_tickets]
GO
CREATE TABLE t_active_tickets (
 ...
 CONSTRAINT 
	[PK_t_active_tickets] PRIMARY KEY(id_ticket ASC))

/**************************************************************/
/*   RMP\Config\DBINstall\MetraPay\Queries.xml                */
/**************************************************************/

PRINT N'Create [dbo].[t_pending_ACH_trans]'

DROP TABLE [dbo].[t_pending_ACH_trans]
GO
...
/**************************************************************/
/*   RMP\Config\DBINstall\Adjustments\Queries.xml             */
/**************************************************************/

PRINT N'Create [dbo].[VW_AJ_INFO]'

DROP VIEW VW_AJ_INFO
GO
