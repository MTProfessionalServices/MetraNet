...
#include "MTTreeRev.h" // MetraNet version
...
LRESULT CALLBACK windowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    case WM_SIZE:
    {
      ...
      MoveWindow(GetDlgItem(hwnd, 1000), 0, 0, LOWORD(lParam), HIWORD(lParam) - 0, TRUE);
      return(0);
    }
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow)
{
  ...
  if (OleInitialize(NULL) == S_OK)
  {
    ...
    productName = "MetraNet";
    title = (char*)malloc(strlen(productName)
                         +strlen(MT_MAJOR_VERSION_STR)
                         +strlen(MT_MINOR_VERSION_STR)
                         +strlen(MT_REVISION_STR)
                         +5);
    strcpy (title, productName);
    strcat (title, " V");
    strcat (title, MT_MAJOR_VERSION_STR);
    strcat (title, ".");
    strcat (title, MT_MINOR_VERSION_STR);
    if ( MT_REVISION != 0 ) {
      strcat (title, ".");
      strcat (title, MT_REVISION_STR);
    }
    
    // Create a Main window.
    if ((MainWindow = CreateWindowEx(0, &ClassName[0], title, 
      1*  WS_CAPTION      |  1*  WS_BORDER       |  0*  WS_DLGFRAME      |
     0*  WS_VSCROLL      |  0*  WS_HSCROLL      |  1*  WS_SYSMENU       |
     1*  WS_THICKFRAME   |  0*  WS_MINIMIZEBOX  |  0*  WS_MAXIMIZEBOX   |
     0*  WS_POPUP        |  0*  WS_CHILD        ,
              CW_USEDEFAULT, 0, 805, 590,
    ...
    //          HWND_DESKTOP, NULL, hInstance, 0)))
    {
      ...
      if ((msg.hwnd = CreateWindowEx(0, &BrowserClassName[0], 0, WS_CHILD|WS_VISIBLE|0*WS_HSCROLL,
                0, 0, 0, 0,
                MainWindow, (HMENU)1000, hInstance, 0)))
      {
        ...
        //DisplayHTMLPage(msg.hwnd, "C:\\Projects\\SlickSkins\\skins\\html\\metratech\\MetraNet Product Install.htm");

        // FIXME this is hacky but we only need to support opening a file
        // relative to the current directory, which is always going to be
        // the top-level directory of our CD-ROM drive -- Simon 11/4/03
        char* cwd = getcwd(NULL,0);
        char* path = (char*)malloc(strlen(cwd)+strlen(lpszCmdLine)+2);
        strcpy (path, cwd);
        if ( cwd[strlen(cwd)-1] != '\\' ) {
          strcat (path, "\\");
        }
        free (cwd);
        strcat (path, lpszCmdLine);
        DisplayHTMLPage(msg.hwnd, path);
