...
CScriptedFrame::~CScriptedFrame()
{
   ...
   if (m_pMTScriptingObject != NULL){
      m_pMTScriptingObject->Release();
      m_pMTScriptingObject = NULL;
   }
...
******************************************************************************/
BOOL CScriptedFrame::InitializeScriptFrame()
{
...
void CScriptedFrame::RunScript()
{
   ...
   m_pMTScriptingObject = new CMyObject();
   m_pMTScriptingObject->AddRef();

   //Add the name of the object that will respond to the script
   m_Engine->AddNamedItem(L"MyObject", SCRIPTITEM_ISSOURCE | 
      SCRIPTITEM_ISVISIBLE);

   //Add the name of the object that will respond to the script
   m_Engine->AddNamedItem(L"MTScripting", SCRIPTITEM_ISVISIBLE);
   
   //Pass the script to be run to the script engine with a call to 
   //ParseScriptText
   HRESULT hr = m_Parser->ParseScriptText(theScript, L"MyObject", NULL, NULL, 
      0, 0, 0L, NULL, NULL);
   if (FAILED(hr))
      ...
      IUnknown **ppunkItem, ITypeInfo **ppTypeInfo)
{
   ...
   if (!_wcsicmp(L"MTScripting", pstrName)){
      if (dwReturnMask & SCRIPTINFO_IUNKNOWN){
         m_pMTScriptingObject->QueryInterface(IID_IUnknown, (void**)ppunkItem);
         return S_OK;
      }
      else if (dwReturnMask & SCRIPTINFO_ITYPEINFO){
         return m_pMTScriptingObject->GetTypeInfo(0, 0x0, ppTypeInfo);
      }
   }
...
void CScriptedFrame::ReloadScript()
{
   ...
   scriptStream.close();
	...
	WCHAR theScript1[8192];
   if (!MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, str, -1, theScript1, 8192)){
	   OutputDebugString("Error translating string.\n");
	   return ;
   }

   m_Parser->ParseScriptText(theScript1, L"MyObject", NULL, NULL, 
      0, 0, 0L, NULL, NULL);
